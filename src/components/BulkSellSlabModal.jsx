...
import { markSlabsAsSoldBatch, notifySlabsLark } from '../lib/supabase'

// ============================================================================
// BulkSellSlabModal — record sales for N queued slabs in one go
// ============================================================================
// Mirror of BulkSellModal (singles). Differences:
//   - Reads from `slabs` schema (cert_number / item_name instead of
//     card_name / set / form / cert / tcg_id)
//   - markSlabsAsSoldBatch writes to slabs table + slab audit log
//   - Lark notification type = 'bulk_slab_sold'
// ============================================================================
...
export default function BulkSellSlabModal({ slabs, currentUserId, currentUserName, addToast, onCancel, onSold }) {
  ...
  const [rows, setRows] = useState(() => slabs.map(s => ({
    id: s.id,
    slab: s,
    sale_price_usd: '',
    sale_fees_usd: '',
    sale_channel_override: '',
  })))
  ...
  const totals = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      const c = r.slab.acquisition_cost_usd != null ? Number(r.slab.acquisition_cost_usd) : null
      if (p > 0) { sale += p; rowsWithPrice++ }
      fees += f
      if (c != null) cost += c
    }
    return { sale, fees, net: sale - fees, cost, realizedPl: (sale - fees) - cost, rowsWithPrice }
  }, [rows])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await markSlabsAsSoldBatch(entries)
      if (result.ok.length > 0) {
        ...
        const totalCost = result.ok.reduce((s, c) => s + (Number(c.acquisition_cost_usd) || 0), 0)
      ...
      if (result.failed.length === 0) {
        addToast?.(`Sold ${result.ok.length} slab${result.ok.length === 1 ? '' : 's'}`, 'success')
        onSold?.(result.ok)
      } else {
        addToast?.(`${result.ok.length} sold, ${result.failed.length} failed — see console`, 'error')
        console.error('[BulkSellSlabModal] partial failures:', result.failed)
        onSold?.(result.ok)
      }
    } catch (err) {
      console.error('[BulkSellSlabModal] submit failed:', err)
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2 text-green-300">
            ...
            <h3 className="font-semibold text-base">
              Batch sell — {rows.length} slab{rows.length === 1 ? '' : 's'}
            </h3>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          <div>
            ...
            <input
              type="date" name="sale_date" value={common.sale_date}
              onChange={handleCommon} required disabled={submitting}
            />
          </div>
          <div>
            <label className="block text-xs text-gray-400 mb-1">Default channel</label>
            <select
              name="sale_channel" value={common.sale_channel}
              onChange={handleCommon} disabled={submitting}
            >
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">
              Buyer <span className="text-gray-500">(applies to all)</span>
            </label>
            <input
              type="text" name="buyer_name" value={common.buyer_name}
              onChange={handleCommon} disabled={submitting} placeholder="—"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto -mx-5 px-5 mb-3 border-y border-vault-border">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r, idx) => {
                const s = r.slab
                const costEach = s.acquisition_cost_usd != null ? Number(s.acquisition_cost_usd) : null
                const priceNum = parseFloat(r.sale_price_usd) || 0
                const feesNum = parseFloat(r.sale_fees_usd) || 0
                const netUsd = priceNum - feesNum
                const realizedPl = (priceNum > 0 && costEach != null) ? netUsd - costEach : null
                return (
                  <tr key={r.id} className="border-b border-vault-border/50 last:border-0">
                    <td className="px-2 py-2">
                      <div className="text-white truncate max-w-[360px]" title={s.item_name}>{s.item_name}</div>
                      <div className="text-gray-500 text-xs font-mono">{s.grading_company} · #{s.cert_number}</div>
                    </td>
                    <td className="px-2 py-2 text-right text-vault-gold text-xs">
                      {costEach != null
                        ? `$${costEach.toLocaleString(undefined, { maximumFractionDigits: 2 })}`
                        : '—'}
                    </td>
                    <td className="px-2 py-2">
                      <input
                        ...
                        placeholder="—" className="text-right text-xs"
                      />
                    </td>
                    <td className="px-2 py-2">
                      <input
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit" disabled={submitting || !allFilled}
