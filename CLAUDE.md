...
- **新 `inventory-sync/surplus_sku_probe.py` 已挂日巡**。判据:**连续 3 场以上正差 + 该产品有同名不同形态的兄弟 SKU = 数记串了**。实测 15 组常驻多出里 **13 组有兄弟**,只有 5 组是真查无来源(Weiss Blue Archive · [JP] OP17 盒 · Gem Vol 6 · DB ST01 · Guardians Rising)。每条给证据:兄弟行是谁、本房有几件、**两个名字前多少字符完全一样**。
