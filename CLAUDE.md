...
- **🔴 但它当场抓到一条评审没提的真问题(既有缺陷,不是这次引入,已知未修)**:`Mega Charizard upc` 排第一的是 **`Charizard Ultra-Premium Collection`(市价 $589 的老版)**,而对的 `Mega Charizard X ex UPC` 排第二。病根在打分:**候选少了输入里的一个身份词(mega)只丢一点分,而候选多带无关词(x / ex)被 Jaccard 惩罚得更狠** → 少了 `mega` 的反而赢。改前之所以「碰巧对」,是因为行尾的 `460` 当时还留在名字里干扰了分数。**实测 exact 匹配 0 个 → 不会自动预选,必须人点**(`BuyListIntake` 只在唯一完美双向匹配时预选),所以不会自动记错账;但**第一行就是错的,而读编号列表的人只认真读第一行**(8/20 tg_move_bridge 那条教训)。**打分函数是全仓最不该随手改的东西,今晚不动**,单独立项。
