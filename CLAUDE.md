...
- **而且不用接任何新数据源**:缓存的发票详情自带 `amount_due`。56 张里 **46 张已付清($217,753)· 10 张未付($35,584)**。
- `inventory-sync/gts_three_way.py`(只读)。四个状态,**只有一个是警报**:
  ```
  已付 + 已入账          1 张  $9,958
  🔴 已付 + 没入账      10 张 $54,899   <- 钱出去了, 系统不知道这批货
  ⚠️ 未付 + 已入账       3 张 $23,743   <- 货在手上钱还欠着(应付账款, 另一个人的事)
  未付 + 没入账(在途)   6 张 $10,149
  ```
- **$54,899 是已经付过钱、系统里一条记录都没有的** —— 最大三张 `INV01150557 $13,754` · `INV01154080 $12,654` · `INV01136166 $10,166`。**这些货卖出去报的毛利都是假的(没有成本)。**
...
- 已挂进 `run_gts_ingest.cmd`,**紧跟在扫描后面** —— 付款状态刚刷新完就对,贴着"钱变动"这个事件而不是每天空扫。
...
- 待定:要不要把 nightly 改成**已映射的自动入账、未映射的单独报出来**。draft-first 是 7/10 定的设计,但"第二步靠人"这三个月里跑了 2/20 —— **没人做的第二步等于没有第二步**。
