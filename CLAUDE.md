...
- **它的前提是反的,而且反得会自己开火。** 全系统 `cost` / `cost_usd` 存的都是**行总额**(`supabase.js:98` 注释原话「cost_usd is the total for the whole order」· 六个 insert 点写 `cost: totalCostJpy` / `cost_usd: totalCostUsd` · `PurchasedItems` 的 `price_mode='unit'` **保存前就乘了数量**)。而它第 66 行 `patch(..., {"cost":unit, "cost_usd":unit})` 把单价写进这两列、**数量一个字不动**,下游 `fetchCostReference` 再除一次数量 → 被它改过的行报出来的单价是 `总额 ÷ 数量²`。
- **🔴 而它的探测条件本身就会命中正确的行,这比「契约相反」严重得多**:`cost > market*2` 是拿**行总额**去比**单件市价**,所以任何按市价买超过两件的诚实进货都会中 —— **929 条在册进货里 600 条光靠算术就够格**;接着 `cost/qty` 正好落回 `[0.25x, 2.0x]` 波段,于是进的是**自动修**那一列不是人工复核那一列。真跑 `--write` 会把大半本进货账压平,**而且压平之后每一个数看起来都还是个合理的价格**。
- 波段判定还**不认币种**:拿本币的 `cost` 比 USD 市价,554 条日元行因此单位高出约 150 倍被跳过。**那是运气不是保险**。
- **✅ 已改成一进来就 `SystemExit(2)`**,`--write` 实测退出码 2、零读零写;原始代码原样留在守卫下面(删掉就没人知道它会干什么了),文件头写清楚重新启用前要先重推前提、并补上备份/乐观锁/回读。
- ⚠️ 它同时违反写库家规:**无备份、无乐观锁、无回读、从不盖 `last_updated`**。

### ✅ Ask 1b「它到底有没有以 `--write` 跑过」—— **没有,而且这是量出来的不是推的**
- 方案文档写的是「查不到:无日志、无计划任务、无引用」。**查不到不是答案**,所以去问数据。
- **它是不认日期的 —— 每次跑都走全表**。所以只要还剩一条老行拿着真总额,它就没跑完过。`fix_unit_costs_forensic_0904.py` 拿 `inventory.avg_cost_basis`(**逐件、且由另一条代码路径在入库时写的,和被怀疑的那列互相独立**)当锚:**326 条行、最老到 2026-05-22,至今拿着完整行总额**。**结论:从来没有跑完过一次 `--write`。**
- ⚠️ **顺带记下一条弱证据别当强证据用**:「554 条日元行里 `cost == cost_usd` 的有 0 条」听起来很硬,其实**那些行根本进不了候选集**(见上面的币种盲区),证明不了任何事。
- **🔴 但这条取证白捡到一个真问题:39 条进货把单价打进了总额栏,COGS 少记约 $87,436。** 最大几条:`Prismatic Evolution ETB 108 个记 $135`(应约 $13,230)· `PRB-02 30 盒记 $315`(应约 $10,200)· `151 散包 400 个记 $25`(应约 $9,784)。**33/39 在 6/24 那个 unit/total 开关之前,和手册里 OP-16、Journey Together 那一类完全同形**;但 **6 条在开关当天或之后,其中 `2026-08-27 White Flare ×4 记 $13.40` 已经是两个月后 —— 说明这一类没有被 6/24 关死。**
- **⚠️ 一个字都没写。** 锚是 `avg_cost_basis`,它自己也可能错;这 39 条是**待核清单不是结论**,明细在 `data/fix_unit_costs_forensic_0904.json`。

### ✅ Ask 1c 迁移前基线已冻结(`data/cost_baseline_0904.json`)
- **硬的那一半(定义上不含任何单位假设)**:在架 **16,139 件 / 278 行 / 账面 $182,283**;**69 行 / 541 件零成本**(对账面贡献 $0)。按房:日本仓 $51,629 · Master $48,042 · PK $45,976 · Front $22,296 · e1 $10,817。
- **软的那一半(每个数都带单位假设,所以假设印在旁边)** 8/01–9/03:门店 $36,061 / COGS $22,851 / **毛利 12% · 成本覆盖 72%** · 单卡门店 $19,752 / **41% · 覆盖 54%** · **slab 覆盖 0%**(8/10 定的:自送评的不填成本)。**Mystery Game 154 行 / $30,889 走 `mystery_game.py` 剔出**,绝不在这里重写一遍判定。
- **每个毛利都必须挂着成本覆盖率一起报** —— 9/2 定的,单看毛利就是在读空气。
- ⚠️ `platform_sales` 窗口内 **0 行**:TikTok / eBay 直播的收入根本不落这张表,**那是「没有」不是「零」**。

### 🔴 Ask 2 跑 `add_sku_units_2026_08_20.sql` —— **跑不了,而且原因终于证死了**
- 迁移**确实一行都没上线**:四个列全 HTTP 400、`product_contents` 和 `products_missing_unit` 全 404。
- **`inventory-sync/data/` 里其实有一个能跑 DDL 的 Supabase 管理令牌**(手册 8/24 写的「DDL 没有自动化路径」已经过时)。新 `sb_ddl.py` 走 Management API,默认 dry-run,`--run` 才发。
  - ⚠️ 两个令牌文件只有一个是真的:`_supabase_mgmt_token.json`(8/18)里存的还是**模板占位符**,所以报的是 `JWT could not be decoded` 而不是干脆的拒绝 —— **一个没填完的配置看起来和一个被吊销的令牌一模一样**。真的那个是 `_sb_mgmt_token.txt`。加载器现在**按形状挑**(`sbp_` + 40 位十六进制),不再按文件名赌。
  - ⚠️ 裸 urllib 打 `api.supabase.com` 回 **403 error code 1010 —— 那是 Cloudflare 封客户端签名,不是 Supabase 拒令牌**(8/13 那条教训第二次兑现)。带上正常浏览器 UA 就过。
- **🔴 然后它把 8/24 那个结论钉死了:令牌能用,但这个账号下只有一个项目 `gmtgoebxathyotgvsskc「lucky4707's Project」(空的)—— 生产库 `dqreqevbjszercgackuc` 根本不在这个账号上。** 这不再是「Gary 进不去后台」的印象,是一次 API 调用的返回值。**Ask 2 和另外 9 张积压的表,全部卡在同一件两分钟的事上:让 William 把 org 的 Owner 邀请给 zhc091@gmail.com。**
- **⏳ 想在空项目上预演一遍迁移,没成**:那个项目报 `ACTIVE_HEALTHY`,但数据库连接四次全部 `HTTP 544 connection timeout`。**所以这份 SQL 至今没在任何一个真 Postgres 上跑过** —— 照实记,别当成「验过了」。
- **✅ 但能离线验的那部分验了,而且当场抓到一条会让迁移当天失败的**:`validate_unit_backfill_0904.py` 把 78 条待回填行喂给迁移自己的 CHECK —— **78 条全部会被 `products_unit_witnessed_chk` 拒绝**,因为它们有 `packs_per_unit` 却没有 `unit_verified_by` / `unit_verified_at`。**证人栏是这条迁移的整个立意**(「A number with no witness is not a number」),所以不能删约束,**得让装载器填证人**。这 78 条的来源本身很硬(42 条「一个包就是一个包」· 27 条「盒上的 packs_per_box 本来就是包数」· 8 条推导 · 1 条按价),**建议证人写 `auto:unit_backfill_0820` 这种一眼看得出不是人的字符串** —— 留空过不了闸,编一个人名是撒谎。**这条要财务侧定,我没有替他们选。**
  - ⚠️ 这个校验器第一版**自己就是空转的**:猜错了 JSON 的键(真的是 `derived`),读到 0 行然后印了一个漂亮的 PASS。已加「读到 0 行就 abort」。**跑不起来的测试和通过的测试,在 CI 之外长得一模一样。**
- 99 条要人判的(账面 $40,431)最大几个:Blue Archive 盒 126 件 $5,120 · Destined Rivals ETB 35 件 $4,900(ppb 写着 9)· Ayakashi 盒 108 件 $3,510。

### ⚠️ 顺带:财务侧今天那个分页修复,病看对了,药少了一味
- `per_stream_pnl.sb()` 原来一次 `limit=50000` 就返回,而 **PostgREST 服务端硬顶 1000** —— inventory 1,649 行只拿到 1,000,**87 个有成本的产品静默隐形、毛利虚高**。这条是真的,改对了。
- **🔴 但新的翻页没有 `order=id`** —— 正是手册 8/20 记过的那条(某脚本返回 5,648 行而只有 5,420 个不重复 id)。**截断是丢行、少算;重复是多行、多算,而多算的方向恰好和这次要修的方向相同。**
- **实测 5 轮 inventory 全部 1,649 行 / 0 重复,账面稳定在 $182,283** —— **所以这是风险不是现行缺陷,按手册「不按行数推严重度」的规矩记为风险**。但 inventory 是全库更新最频繁的表之一,而修法只是加一个 `&order=id`。
...
- **🔴 而真正该修的是:掉了没人扶。** 两个计划任务**都只有「登录时触发」**,而且 `LV Singles Webapp Fixed` 状态是 **Ready(没在跑)**,底下那个 python 是 08/20 手工起来之后**脱管**跑到现在 —— **Windows 根本没在看着它**。**它要是死了,就一直死到有人重新登录为止。那正是 8/12 那次挂了一整天的形状。**
- **✅ 新 `lv-singles-erp/singles_watchdog.py` + 任务 `LV Singles Watchdog`(每 5 分钟)已上线并实跑通过**(Result=0)。两个检查分开,因为**它们坏的原因不同、要找的人也不同**:`127.0.0.1:8080/health`(app 活着吗)· 公网 `/health`(隧道在带流量吗)。**app 死 → 重启 app 任务;app 活但公网不通 → 重启隧道任务。**
- **写死的三条安全线**:① **重启只按任务名,杀进程只按端口查出来的 PID** —— 8/11 那次按命令行关键词杀,把 8081 的 slabs 一起杀了(它的命令行里没有 "slab" 这个词)② **脚本显式拒绝杀 8081 的 PID**,测试里实测拒绝了 pid 8248 ③ **`/health` 拿到 401 算「活着」不算「挂了」** —— 前面哪天加了密码不能读成停机。
- **探测失败要重试 3 次**:隧道重连窗口是 ~15 秒,**把正在自愈的 15 秒报成停机、然后去重启它,只会把停机拉长**。
- **只在状态翻转时发 Telegram**(坏了 / 回来了)。**每 5 分钟报一次「一切正常」的看门狗是墙纸,而墙纸就是真消息被漏掉的原因。**
- **`test_singles_watchdog.py` 逐个分支实测**:健康时不动任何东西且不出声 · app 挂了只重启 app 不碰隧道 · 隧道挂了只重启隧道不碰 app · 恢复只播报一次 · 401 读成活着 · **拒绝杀 slabs**。**7 项全过,而且「拒绝了」是靠 stub 记录到的空清单证明的,不是靠打印一句拒绝。**
...
- 三条改动,**顺序本身就是修复**:① 行尾金额先摘掉进 `note`,**永不当价格也永不当数量**(店里写的是市价,当成实付会记 $7,506 而真实 $3,000)② 显式 `xN` / ` - N` 压过开头整数 ③ 套号合并限定 1–2 位 + `LONG_CODE_RX` 拆三位以上,让 `sv151` 和目录里的 `SV 151` 归一到同一组 token。
- **🔴 Codex 这一轮抓到金额正则太窄,是真的**:`$1,800 USD` 匹配不上 → 金额留在行上 → `xN` 不再在行尾 → **开头整数兜底读成 151 件**。**一个没认出来的币种词,两条规则之后变成一个错的数量。** 已放宽到 `US$` / `USD` / 行尾句点,并把四种写法钉进测试。
- **决定性验证还是双版本对跑**(`scratchpad/parser_regression.mjs` **已重建** —— 9/01 那份没留下来):线上版 vs 新版跑全部 **864 个在售名字**,`parseBuyList` **864/864 完全一样**(0 个 qty 变、0 个名字变)· **自己找得到自己 863/864,改动前后一致,新增 0 丢失 0** · **top-1 移动 0 个**。另跑 25 条对抗输入:**11 条变了,0 条变差**。测试 90 → **105 项**。
- **⏳ 两条已知、都没改,要你定:**
  - **🔴 开头是裸数字时仍当数量**:`151 booster bundle` → **qty=151 / 名字「booster bundle」**。**这条线上版本也一样,不是这次引入的**,但它和这次新加的「三位数字是套名不是套号」规则自相矛盾。**没法自动判**:`100 booster packs` 读成 100 件是对的,而 `151 booster bundle` 读成 151 件是错的 —— 分辨要查目录,而 `parseBuyList` 是**故意不认识目录的**(它拿不到 products)。**要么规定门店对 151 必须写 `x1`,要么让 BuyListIntake 在 qty 来自开头整数时高亮要人确认。**
  - **⚠️ `Mega Charizard upc` 排第一的仍是老版 `Charizard Ultra-Premium Collection`,对的排第二**。**打分函数的既有缺陷**(少了输入里的身份词只丢一点分,多带无关词被 Jaccard 罚得更狠);改之前碰巧排对,只是因为行尾的金额还在污染分数。**exact 匹配 0 个 → 不会自动预选,必须人点**,所以不会自动记错账。**测试钉的是「对的那行必须在候选里」而不是钉住错误顺序** —— 钉住错的顺序就是 8/26 那个「旧断言正好在要求这个 bug」。打分函数单独立项。
- ⚠️ 金额只认行尾:`Prismatic spc $510 x2`(金额在中间)会把 `$510` 留在名字里。**线上版本也一样**,不是回归;全库 891 个产品名带 `$` 的是 0 个,所以真要治也安全,但不在这次范围。
...
- **表本身在我们碰过的地方确实变清楚了**(新 `pk_count_sheet.py` 逐行重建 counter 手机上看到的样子):PK 现在 **33 行 / 1,114 件**;**OP-15 三行从「前 37 字一样」变成「第 1 个字就分开」,Marvel 两行从 60 → 0**,而且**没有被打散,还是并排**(行距 1)——「形态放最前会把同一个套拆到表两头」这个担心没有发生。
- **🔴 但盘点难不难和这张表基本无关,数据指的是另外两件事**:
  - **① 数成 0 —— 9 天里 $101,455 账面就这么没了**(新 `pk_zero_writes.py`)。最贵一笔:**8/28 Yaz 把 OP-17 EN 盒 176 → 0**。⚠️ 这个金额被虚高的 basis 撑大了($353.84 而实付 $72,8/21 已记),**按实付算那一笔是 $12,672 不是 $57,904,九天真实口径约 $56,223** —— 报这个数一定要带这句,否则是拿一个已知错的成本吓人。
  - **② 单向棘轮 —— 9 行卡住,79 件 / $23,603 实物在架、账上没有**。形状完全一样:某一场把账写低(多半是数成 0),之后每一场都报同样大小的多出,而正差永不写库。`BOX · [EN] OP-15`(账 0 / 每场数到 15)· `CASE · [JP] OP-17`(账 0 / 数到 5)· Hololive(账 48 / 数到 59)· Blue Archive(账 13 / 数到 18)。**改名治不了任何一行。**
- **🔴 所以「点货越来越烦」烦的是消息不是表**:表一直 33 行左右,**多出行数 5 → 13**,而最后一场 **13 行里 12 行是同样的行又报一遍,只有 1 行是新的**(`pk_surplus_trend.py`)。最后一场多出合计 **130 件 / $33,513**。
- **✅ 破了 PK 那串疯狂数字**(`[JP] OP17 盒` 连续 9 场 +193/+137/+70/+72/+65/+59):**OP-17 散包行账面是 0 → 按 `.gt('quantity',0)` 根本不在点货表上 → 数散包的人只好把数字写在唯一看得见的 OP-17 行,也就是盒那一行。** 9/2 有人给散包行补了 417 之后,它当场开始正常(417 → 300 → 282 → 261,像真销售),盒行的疯狂数字同时停了。**这就是「归零行 48 小时留在表上」(`feat/zero-row-grace`,8/21 提过没做)最硬的一个病例。**
- **OP-17 全族:8 个 SKU / 5 个有货 / 三种写法**(`op17_family.py`)。**Front Store 那两行 62 个字只差第 2 个字母**(`[EN]` vs `[JP]` … `(OP17) Booster Box`),而 basis 是 **$353.84 / $333.00,同一个日文盒另一行记 $107.20 —— 差 3 倍**。这正是卡在 Frank 那里的合并决定,**它同时是全 PK 最贵也最常出错的一族,该优先解**。
- **48%(16/33)的名字在手机上读不完**,最长 88 字;3 行海贼王看不出是哪个套;3 行库里名字把形态词写了两遍。
- **⚠️ 唯一一条「看起来最糟但实测没在错」**:`[EN] OP-16`(72 件,显示出来只有这 5 个字,因为 `extractLaunchName` 把结尾的 category 剥掉了)对 `[EN] OP-16 The Time Of Battle Sleeved Booster Pack`(318 件)—— **一个名字是另一个的开头**,390 件。但逐场查下来两行都数得准。**记为风险,不当缺陷报** —— 手册里已经有过「按行数推严重度」的教训。
- **🔴 点货表显示的名字 ≠ `products.name`**:`StreamCounts.jsx:38` 的 `extractLaunchName` 会**去掉结尾的 `products.category`**。所以判两行分不分得开,**必须按剥完之后的字符串算**,按库里的名字算会得出偏乐观的结论。

## 🔴 9/2 常驻多出的真病根:不是货多出来,是数落在了同族的另一行(Gary:「这个我们需要修 或者daily probe建立sku」)
- **新 `inventory-sync/surplus_sku_probe.py` 已挂日巡**。判据:**连续 3 场以上正差 + 该产品有同名不同形态的兄弟 SKU = 数记串了**。每条给证据:兄弟行是谁、本房有几件、**两个名字前多少字符完全一样**。
- **🔴 当天写的「只有 5 组是真查无来源」是错的,当晚查 Packheads 点货表时推翻,已修**:那 5 组里 **`[JP] OP17 盒` 和 `DB ST01` 其实有兄弟,只是同一个套在我们目录里写了三种写法**(`… The World's Strongest Warriors (OP17) Booster Box` / `BOX · [JP] THE WORLD'S STRONGEST WARRIORS` / `OP-17 The World's Strongest Blister Pack`),`base_key` 按名字归族就把一个货架劈成了三家,谁都不是谁的兄弟。**这正是最危险的方向:把「你数落错行了」报成「查无来源」,让人去做一次没有意义的实物复点。** 已加第二条索引 `set_code()`(`OP-17`/`(OP17)`/`OP 17` 归一,9 条用例全过,**没套号的产品行为完全不变**),真查无来源 **5 → 3 组**;新增第三个分段 **`🔀 同一个套写法不统一`** —— 这类不是「看串行」,名字长得完全不一样,是**数货的人根本看不出这几行是同一堆货**,处方也不同(先统一写法,不是把两个名字改得更不像)。
- **🔴 它不建 SKU,因为建了没用 —— 9/1 那次 Marvel 修复被第二天的盘点直接推翻**:我建了散包 SKU + 写了 24 件,而 9/1 20:09 Yaz 把包行数成 2(账 24)→ **−22 写库把它抹回 2**,同时把 25 件全记在盒那行。**四场盘点、三个人,一模一样。** 病根是两行叫 `…Hobby **Pack** Booster Box` 和 `…Booster **Pack**`,**盘点表上都带 Pack,第一个不同的字在第 60 位**。**创建 SKU 不是修复,让两行读起来不一样才是。**
- **✅ 命名规范定案:`<形态> · [语言] <套号> <套名>`,形态放最前。** 我第一版把标记放在末尾(`… — BOX (32 packs inside)`)—— **探针当场测出两个名字前 52 字符还是一样的**,而「放末尾能让两行排在一起」这个理由已被证伪:那两行并排坐了 11 场,三个人照样合并。第二版提到最前,**第 1 个字符就分开**。
- **探针两次抓到我自己的错**:① 我的改名后缀让家族识别失效 —— **建议改名的工具,不能在别人照做之后就认不出来了**(已改成取分隔符两侧最长的那段,而不是往词表里加 `LOOSE`/`SINGLES` 打地鼠)② 修的时候剥掉了所有数字 → **`Gem Pack Vol 6` 和 `Vol 5` 被判成同族**,这个方向最危险:会把真查无来源说成「你数错行了」。已改成不剥数字(`(32 packs)` 括号规则已处理),**7 条家族判定用例全过**。
- **探针分三段,改完的不再重复喊**:`✅ 名字已改到分得开(等下一场自己断)` / `🔁 还需要改名` / `❓ 真查无来源`。不分的话第二天看到同一行会以为什么都没发生 —— 和 8/12「已决定却被反复要求再决定」同病。

## ✅ 9/2 改名已落三批(Gary:「改 和frank 确认一下再apply」)
- **Marvel 两行**(改了两次,第一次标记在末尾是错的)· **OP-15 三行 + [JP] OP-17 三行**(Frank 15:08 批的范围:「Dont touch white flare, ascended, crown zenith, op 08, hololive, op11 lets focus on op17 and op15」)。旧名全进 `aliases`,**`sku_urls.json` / `ebay_bin_queries.json` 新 key 加进去、旧 key 保留**(7 月改名删旧 key 废掉 90/286 条钉价)。
- **apply 跑的是 Frank 看过的那份 json,不是重新生成的** —— 重新生成会悄悄改变他批准的范围。
- **⏳ 海贼王全量统一提案已发 Frank**(`data/op_rename_proposal_0902.json`,有货 33 个 / 29 个有建议名)。**🔴 卡在一处**:两个 JP OP-17 盒(各 17 件)补上套号后名字会**完全一样** —— 那正是手册里「同物异名待合并」那对。**先定合不合并,再改名**,否则会造出一对逐字节相同的 SKU。
- **发提案前必过重名闸**:提案名不能撞现有产品、也不能两行撞一起。**让人批准一份 apply 不了的清单,是在浪费他的注意力。**
- **家规救了一次重复回复**:按「答成员问题前先读 `tg_chat_log.jsonl`」查了,**runner 已替我回过 Frank 三条**(OP17 卖 $14 是 TikTok 平台券 $5.85 不是我们打折 · OP-05/EB-02 成本已写进 `recap_costs.json` · 改名范围已确认)。没看就答就是 8/22 那次重演。

## 🔴 9/2 权限:Juney 事件的病根不是「谁有权限」,是这个页面没有会话身份
- **Juney 9/2 19:00 UTC 在 Master 提交盘点,115 行 / 74 行负差 −1,686 件 / total_sold=1686**,已撤回(理由:本该是 RocketsHQ)。**库存已完全还原 —— 74 行全部回到盘点前账面,净缺口 0 件**(单独核过,没只信撤回记录)。
- **🔴 但「不给主播 Master 权限」治不了它:Juney `can_login=False`,她根本登不进 app。** 名字是**别人在下拉框里选的** —— 全库 123 场盘点里 117 场记在登不进系统的账号名下,34 个人只是下拉框里的名字。**而 `stream_counts` 只存「选了谁」不存「谁提交」,所以那次是谁点的按钮,现在查不出、以后也查不出。**
- **✅ 已收权(改数据,立即生效)**:**Mich / Mike 离职撤权**(两人本就 `active=False` + 无 PIN、双重锁死,这次抹掉行上残留的 `can_login=true` / `allowed_pages=全部` / Mich 的 `high_value_access` —— 免得哪天 active 被翻回来全权限就回来)· **`/manual-inventory` 收回到 Eric/Gary/Aldo**(Sully/Mario/John John/Will/Admin/JAY/Streamer 七人)。
- **⚠️ 我上一条把风险说大了**:我说「三个能登录的账号没有 PIN」时**只筛了 `can_login` 没看 `active`**。登录条件是 `.eq('pin',pin).eq('active',true)` 再过滤 `can_login!==false` —— Mich/Mike/John John 都没 PIN,本来就登不进去。**那个洞当时并不存在。**
- **Sully 的日常没受影响**:她 7/1 以来 124 笔 Master 动作**全是 Transfer,116 笔是 Master→门店补货**,从没盘点过也没手工调过库。Gary 选的是「先只禁手工调库」。
- **🔴 `/manual-inventory` 打开时默认就选中 Master**(`ManualInventory.jsx:71-73`),而它直接调 `updateInventory` —— **不写 movements/receipts/acquisitions**。「手工加 Master → 转出来」是阻力最小的路,一半原因是它默认帮你选好了。**这个默认值该拿掉,待发版。**
- **⏳ 还没做(要发版)**:Master 盘点白名单 · 写掉超过 N 件/金额时弹确认 · **`stream_counts` 记录提交者的会话身份**。

## ✅ 9/2 「没有成本」研究完了:它不长成缺口,长成好毛利(Gary:「仔细研究一下我们怎么看比较好」)
- **只看现在还挂着零成本行的产品,在收银台卖掉的部分**:6月 $4,838 收入 / $3,454 成本 = 29% 毛利 → 7月 46% → **8月 $14,604 / $1,088 = 93%** → 9月 78%。**毛利率从 29% 涨到 93%,涨的全是空气。** 好消息没有人会去查 —— 这就是它「总是遇到」的原因。
- **成因(609 件 / 87 行 / 76 个产品 = 件数的 3.6%)**:**A 从没买过 73 行 / 426 件(货从系统外进来,主因)** · D 别处有成本这行没接到 7 行 · C 有成本没走到货架行 6 行 · B 1 行。**C+D 只有 13 行却占可补金额的 82%** —— 钱早在账上,只是没走到货架行,这类最便宜。
- **✅ 已补 17 行 / 109 件 / $5,594**(只用自己的进货单价或别房 basis,查无来源的留空)。**拦下 `Prismatic Evolutions Booster Pack` 99 件没写** —— 进货算出 $11.50、别房 basis $0.95,**差 12 倍是 unit/total 填反的签名**,两个数写哪个都是瞎选。手册 8/07 记过实付就是 $11.50,**待 Gary 一句话补 $1,138**。
- **✅ `cost_recovery_probe.py` 挂进日巡** —— 它 8/12 写好后**从来没接到任何东西上**,又一个「装好了没响过」。⚠️ 第一次写把整个 `.cmd` 从 LF 转成 CRLF(整文件重写不是追加),已从备份还原后按字节追加,验过 `appended-only: True`。
- **怎么看比较好**:① **不看「多少行没成本」,看「毛利里有多少可信」** —— 每个毛利旁边挂**成本覆盖率**;「93% 毛利 + 7% 覆盖率」一眼知道不能读,单看 93% 会以为是好月份 ② **三态不是两态**(有成本 / 能补 / 查无来源),第三类**永远不许自动填** ③ **库存价值永远报两行**,给一个数就等于宣称我们知道。

## 📌 9/2 SKU 体检结论(863 个在售 / 209 个有货)
- **Black Bolt 两行同名都有货 —— 不是重复,是两个产品**(Gary 确认):`6088d502` lang=JP / ¥2,500 从 HUA / 日本仓,`f1ebe9c5` lang=EN / $1,960 从 Frank / 美国房。**该加语言前缀,不该合并。**
- **Dual Evolution 两行确实是两个批价**(Gary 说对了):¥240,000(8/23)vs ¥168,000(8/31),一周跌 30%。**但它同时也是重复 SKU** —— 同产品正常该是一行加权平均 $1,366.80,现在 Master 盘点表上同样的字出现两遍。**🔴 而且两行各有 2 笔进货(`jp_vendor` + `jp_to_us_shipment` 金额相同)—— 那个 $198,879 重复计成本的 bug 到 8/31 还在产生新行。**
- 其余欠账(不改不会天天咬人):**形态词写两遍 14 个**(5 个有货)· **海贼王没语言前缀 21 个**(10 个有货 / $12,215)· **`packs_per_box` 空 32 个有货**(最大是两个 OP17 盒)· **同一个套 3 行以上同时有货 12 族**(Storm Emeralda 一族 4 行 $33,505 最大)。
...
- **🔴 顺带修掉一个既有缺陷**:`storefront_recon` 的双录检测 `sorted(key=lambda s: (s["product_id"], ...))` 在 `product_id` 为 NULL(收银台手录卡)时 **TypeError**,而整段包在 try 里 → 那天整个门店对账不跑。已改成跳过无 product_id 的行(它们本来也不可能「同品双录」)。**实际影响查过日志:29 次里死了 2 次(8/22 和 9/01),不是长期失效** —— 我先前按「6 月以来 131 条 NULL 行」推的严重度是错的,那些行大量落在同一天同一窗口里。**推严重度要去数事件,不要拿行数当天数。**
...
- **✅ 「清单上的数字是市价」判对了,三条独立证据**:① **9 条能查价的里 8 条落在 TCG 市价的 98–101%**(Prismatic SPC $255/$259 · 151 bundle $180/$179 · 151 ETB $560/$569 · Perfect Order $175/$174 · Chaos PC ETB $135/$136 · Mega Lucario PC $230/$231)② **8/26 那条一模一样格式的**:`X1 journey etb $138 … $380 cash paid / $350 from hard box / $30 store front cash` —— 逐行加起来 $602,实付 $380 ③ **Prismatic Booster Bundle 写 $90,正是我们自己 8/26 在收银台卖出的价**。Gary 9/1 确认:「这些是 mp」。
- **🔴 但「实付 = $3,000」判错了,Gary 纠正:「小马付了80%」。成本 = 每行市价 × 80%,总额 $6,104.77($7,630.96 市价的 80%)。** 我把「Paid $3k from hard box」读成了付款总额,**它只是其中一个出款口**,余下 $3,104.77 走了别的渠道。**8/26 那条先例本来就写着三行(`$380 cash paid / $350 from hard box / $30 store front cash`)—— 出款口从来不等于总额,而今晚这条只写了出款口没写总额,我拿一个出款口当了总额。以后看到「from hard box」先问总额是多少,不要从行项目反推。** 顺带纠正:**这批不是「市价的 40%」,是 80%** —— 40% 那个说法只是我把错的总额除以市价的产物,别拿它当买入质量的基准。
- **重记的做法**:成本从「一笔钱按权重分摊」改成**每行独立 × 80%**(更简单,而且每行自洽);basis 一律**从第一次写库前的原始快照重算,绝不拿表里现值再算一遍** —— 同一批行改了三轮,拿自己上一轮的答案当输入会把三次舍入滚进去。
...
- **✅ Pitch Black PC ETB 已并进同一笔(Gary:「是同一个东西 你看看pitchblack的成本 他应该有买的记录」)**:Sully 18:02 加的那 1 个不在店里贴的清单上,Gary 确认是同一笔买入漏写的一行。店里没给它写市价,**用我们自己钉过价的 $124.96 × 80% = $99.97**(和其余 14 行同一口径)。
- **🔴 但「pitchblack 应该有买的记录」的答案是:有,可那是另一个产品。** 7/09 GTS `INV01177509` 一张单买了整族 Pitch Black —— **普通 ETB $31.45/个** · 盒 $95.25 · bundle $16.25 · sleeved $2.77 · 3-pack blister $9.21。**而 `13e5d79c Pitch black elite trainer box pc ETB`(宝可梦中心版)从来没有过任何一条进货记录**,市价 $124.96(钉过价),普通版才 $69.42。**分销商买的是普通版,PC 版是门店零散收的** —— 判 PB 成本时别拿 $31.45 顶上去,那是另一个盒子。
- **🔴 而且这个 SKU 已经这样漏了三次,两次带着 $0 成本卖掉了**:8/14(Sully)· 8/27(Aldo)· 9/01(Sully),每次都是「手工加 Master → 转 Front」,零进货记录;**8/14 以 $40 卖出、8/26 以 $110 卖出,`cost_basis` 都是 0.00,两笔都报了 100% 毛利**。卖出行不回改,只记在这里。⚠️ **数还对不上**:movements 只解释得了 3 进 2 出 = 1,而 Front Store 现在是 3 —— 多出的 2 个没有来路(疑似收银台 `stock_adjust` 覆盖那条路补进来的),**没编成本,那 2 个仍是 $0**,写进去的 $49.13 摊到 3 个上所以行 basis 只有 $16.38(**总金额是对的,单价被两个无主的 0 稀释了**)。
- **⚠️ Lark 采购卡发早了,数字是错的**(Gary:「可以」→ 我发了):走**线上 `/api/lark-notify` 的 `purchased` 类型**,和 `/buy-list` 同一条代码路径、同一个群(Acquisitions Squad),Lark 回 `code:0`。**但那张卡上写的是 $3,000 和「39–40% of market」,而真实是 $6,104.77 和 80%。** 这个群里的人正是按「% of market」校准出价的,**39% 会把以后的出价锚偏一半**。更正稿已备,等 Gary 定发不发(`purchased` 类型没有「更正」形态,再发一张会读成第二笔买入;要发得走浏览器 `lark_send` 发纯文本)。**教训:对外播报要等口径落定再发,别赶在人回话之前。**
- ⚠️ `151 Elite Trainer Box` 原有那 1 个的 $510 basis 本身就可疑(像是把市价当成本记的),另案。
...
- **测试 76 → 90 项,跑真模块 + 变异测试**:把套号上限撤掉 → 2 条红;把开头整数挪回最前 → 2 条红;把紧凑套号拆分关掉 → 3 条红。`npx vite build` 通过。
- **🔴 Codex 第一轮抓到我修出来的一个新洞,是真的**:我把「合并」限成 1–2 位,于是**目录写 `SV 151` 而人打 `sv151` 就再也匹配不上了**(输入 token `sv151`,目录 token `sv`+`151`)—— **修一个方向顺手把另一个方向弄断,而两边都只在三位数上发作**。已加 `LONG_CODE_RX`:前缀后面 3 位以上数字**主动拆开**,两种写法归一化到同一组 token,`op17` 这类两位的照旧原子不动。**教训:归一化改一边就必须两边一起验,不然「对称」这个前提是悄悄断的。**
- **✅ 全目录 before/after 回归(`scratchpad/parser_regression.mjs`,同时 import 新旧两个版本跑真数据)**:855 个在售产品名各自喂回去,**「自己找得到自己」845/855 改动前后完全一样,855 个名字里 top-1 变化的是 0 个** —— 这个改动对现有目录是惰性的。店里那 14 行则全部从「qty 读不出来 / 无候选」变成正确。**这种双版本对跑比任何评审都硬,以后改匹配器就用它。**
- **🔴 但它当场抓到一条评审没提的真问题(既有缺陷,不是这次引入,已知未修)**:`Mega Charizard upc` 排第一的是 **`Charizard Ultra-Premium Collection`(市价 $589 的老版)**,而对的 `Mega Charizard X ex UPC` 排第二 —— **Gary 9/1 亲口确认「Mega Charizard X ex UPC 是对的」,所以这是确认过的错排,不是我的猜测**(我入库时是靠市价 $589 对不上 $230 才没跟着它走的)。病根在打分:**候选少了输入里的一个身份词(mega)只丢一点分,而候选多带无关词(x / ex)被 Jaccard 惩罚得更狠** → 少了 `mega` 的反而赢。改前之所以「碰巧对」,是因为行尾的 `460` 当时还留在名字里干扰了分数。**实测 exact 匹配 0 个 → 不会自动预选,必须人点**(`BuyListIntake` 只在唯一完美双向匹配时预选),所以不会自动记错账;但**第一行就是错的,而读编号列表的人只认真读第一行**(8/20 tg_move_bridge 那条教训)。**打分函数是全仓最不该随手改的东西,今晚不动**,单独立项。
- **Codex 另一条(尾部 `$N` 可能把 `Promo Pack $5` 误配到 `Promo Pack`)查了真数据后不改**:**891 个产品名里带 `$` 的是 0 个**,而 `parseBuyList` 是故意不认识目录的(它拿不到 products),为一个不存在的行给无目录解析器加目录判断反而破坏它的设计。金额也没丢——进了 `note`,会写进 `acquisitions.notes` 并显示在行上。**以后真出现带价格的产品名,这条要重开。**
...
- **匹配器的课**:pack/sleeved/blister 在本目录是一族(sleeved SKU 的 category 就叫 Blister Pack),形态闸门要按族放行;set 码正则的 \b **在 heredoc 里又变了退格符**(第 N 次!两处正则静默失效,靠真数据 ST01 配不上才暴露)——铁律重申:**这条管道改 .py 只用 Write/Edit,heredoc 里的反斜杠一律 chr(92)**。
...
- **🔴 一个反复中招的陷阱,值得单独记**:`bash heredoc → Python → JS` 三层下来,**`\b` 到 Python 时已经是 `\b` 被解释成退格符(U+0008)**。它在正则里不再是词边界,于是 **`!/boxe?s?\b/` 这种否定断言永远为真 —— 测试报平安,但它什么都没测**。今天写坏了三条,Codex 抓出一条。已在 `jp_group_test.mjs` 顶部加守卫:**文件里出现字面退格符就直接 abort**,并对 case 和双袋两组断言各做了一次变异测试(故意改坏代码,确认测试真的会红)。**以后这条管道里的反斜杠一律用 `chr(92)` 构造,不靠小心。**
