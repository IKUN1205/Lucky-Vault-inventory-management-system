...
- **✅ Pitch Black PC ETB 已并进同一笔(Gary:「是同一个东西 你看看pitchblack的成本 他应该有买的记录」)**:Sully 18:02 加的那 1 个不在店里贴的清单上,Gary 确认是同一笔买入漏写的一行。**现金没变,所以 $3,000 改成按 15 行分摊**(市价基数 $7,506 → $7,630.96,系数 0.39968 → 0.39314),14 行成本各降约 1.6%,PB 拿到 **$49.13**;**合计仍然精确等于 $3,000**(备份 `buylist_realloc_backup_0901.json`,乐观锁 + 回读 15 行全 OK,**再一次一个 quantity 都没动**)。基数是从第一版备份里的**原始 basis** 重算的,不是拿表里现值再算一遍 —— 拿自己上一次的答案当输入会把舍入越滚越大。
...
- **✅ Lark 采购卡已发**(Gary:「可以」):走**线上 `/api/lark-notify` 的 `purchased` 类型**,和 `/buy-list` 同一条代码路径、同一个群(Acquisitions Squad),Lark 回 `code:0`。**报的是入账的 $3,000,不是清单上的 $7,506**(报市价会读成我们花了这么多)。市价百分比只挂在 `erp_pricing` 真取到价的 9 行上(**全部 39–40%,口径一致**),另外 6 行卡上明写「没有市价可对」—— **店里自己写的市价数字没有冒充成我们的市价读数**。
