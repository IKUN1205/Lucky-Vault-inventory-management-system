# LV Inventory — 作业手册 brief (2026-09-06)

## ✅ 9/6 夜 OP-17 三步做完,FIFO 第一次给出真实阶梯(Gary:「都做 你来尝试 给我结果 我来告诉你对不对」→「对」)

- **起点是 Gary 一句判断**:「每天价格采购价格是不一样的 他们直播间销售应该消耗之前的 case 先 然后到后面的 case 的逻辑」。**他是对的,而且账当时完全不是这么走的。**
- **阶梯先算出来给他核过(他确认「对的」)**:
  ```
  08-20   7 箱 →  84 盒 @ $206.58      08-24  10 箱 → 120 盒 @ $138.19
  08-21   3 箱 →  36 盒 @ $195.42      08-31   1 箱 →  12 盒 @  $93.80
  08-21   9 箱 → 108 盒 @ $178.67      另:Frank 直购散盒 $325–333 · Chiyoda $107.20
  ```
  **单价从 $206 一路跌到 $94** —— 所以先进先出在这里不是记账偏好,是几倍的差别。
- **① 清 shipment 腿 $61,660.10**(`op17_step1_clear_dupes.py`)。判据是**日方自己的账**(`OP-17-原箱 入库29 = 出库lv29 = 库存0`,一次事件),**不是「找不找得到一条一模一样的行」** —— 4 行里 2 行本来就配不上对,而那正是问题所在。**行保留、运单保留,只是不再自称进货。**
- **② 合并两个 JP 盒 SKU**(`op17_step2_merge_jp.py`):`71cc6ce5 BOX·[JP]`($107.20 Chiyoda)并进 `77d8f781`($325–333 Frank day-one)。**这次动了历史指向,和 8/19 那批不一样** —— 那批 loser 全是零库存零进货,这个有 13 笔真进货 + 4 盒实物,**不搬则保留行多出 4 盒没有进货、13 盒的层搁浅在死 SKU 上,阶梯从另一头断掉**。basis **不跨房混平均**(Rockets 那 4 盒真的是 Chiyoda 批)。
- **🔴 ③ 而第 2 步当场撞出 `OVER_ON_HAND`,这条是整晚最要紧的一课**:开账层是 9/4 冻结的,当时这两个还是两个产品。**合并 SKU 必须连开账层一起合,而开账文件不可改(有 sha256,看门狗每天验)。**
  - **解法:不碰冻结文件。** 新 `data/cost_layer_merges.json` 声明合并,`cost_layers.py` 在**读的时候**套上去 —— **地板一个字节没动,调整显式、带日期、带理由**。保留行拿到的是**第二个层**(13 件 @ $107.20),**不是把两个价平均掉** —— 混平均正是要消灭的那个东西。
  - 结果:`OVER_ON_HAND → OK`,算不出 FIFO 的产品 **4 → 3**、件数 **90 → 70**。
- **📌 FIFO 和账面第一次真的分开了,而差额正是「先烧早的」和「一锅平均」的差**:
  ```
  Japanese OP 17   在架 20   FIFO $3,724.60 ($186.23/盒)   账面 $5,756.80 ($287.84/盒)   差 −$2,032
  English  OP 17   在架 12   FIFO $3,900.00 ($325.00/盒)   账面 $4,101.88               第0层已烧完
  ```
  **EN 那个 $325 是真实当前层** —— $72 的 GTS 批烧尽了,而账面还背着 $207.57 那个混合平均,**每卖一盒少记 $120 成本**。
- **✅ 顺带补上 9/6 早上记的那个盲区:看门狗加了算法指纹。** 开账文件有 sha256、算法没有 —— 改一行 `cost_layers.py`,所有 FIFO 数字一起变而没有任何东西出声。**加完当场抓到我自己这次的改动**(`3d958234f9dd -> 6faacd41c864`)。**它不是锁**(这个文件本来就该改),它只让改动**不能悄悄发生**;确认后自己清掉。指纹取不到时**明说「这一项没跑」,不算通过**。
- **⚠️ 同一次看门狗还对我当天另外两笔写库提出异议,这是它该做的**:`LOOSE PACK Marvel +20 件没有层` · `Gem Vol.5 +3 件没有层` · `FIFO-vs-账面 一天从 −$336 走到 −$10,063`。**两条都是真的** —— 那是手动加的库存,没有对应进货,FIFO 背不动。**看门狗对我今天做的事提出异议,比它保持安静有价值得多。**
- **⏳ 箱→盒那 336 个盒补不了,而且这是决定不是遗漏**:30 箱买进、**28 箱被盘点负差按 $2,144/箱写掉(一次性记 $60,043 COGS)**,而箱不是卖掉的、是拆成 12 个盒的。**`box_breaks` 全表 2 行、都在 7/22、没有一行是 OP-17。** 那 336 个盒早卖光,现在记进去就是 336 个幽灵。**只能往后修:下一箱进来必须走 `box_breaks`。**

## ✅ 9/6 夜 OP-17 两个盒行改名(Gary 给的格式:「English / Japanese OP 17 booster box · 可以结尾配上包数」)

- `English OP 17 Booster Box (24 packs)` / `Japanese OP 17 Booster Box (24 packs)`。**点货表上共同前缀 60 → 0 个字符**(和同族 `BOX·[JP]` / `CASE·[JP]` 两两也都是 0)。
- **为什么非改不可,六场盘点实测**:`09-03 EN 账18 实17 / JP 账0 实27` · `09-04 EN 账12 实0 / JP 账0 实24` · `09-05 JP 账0 实12` · `09-07 EN 账24 实5 / JP 账10 实9`。**JP 的账连续五场是 0,三个人在它上面数到 27/24/18/12;EN 那行被一路写低。** 正差不写库、负差写库 —— **单向棘轮两条腿同时咬住。**
- **顺带 `packs_per_box` 从 NULL 补成 24** —— `DECISIONS.md` 已定,证据来自**我们自己的 EN listing**「1 Display Box: 24 packs」,同一个来源管两个语言版本。**NULL 在下游读作「一件=一包」,那是 30 倍那一类的坑。** 旧名全进 `aliases`;改前扫过所有按名字索引的配置,**零引用**。

## 🔴 9/6 我重问了一个财务线两天前就答过的问题(Gary:「这个问题我们 finance 已经 work 了无数遍了 我们还是联动有问题」)

- 我问「OP-17 到底有没有英文版」,而答案一直在 `memory/CURRENT_STATE.md:36-37`(EN $329 / JP $138.19 两条成本层)、`DECISIONS.md:338`、`OPEN_LOOPS` 的 OL-001 / OL-026。**而 `MEMORY.md` 第一行就写着「开工先读三层」。指路牌在,我没走。**
- **🔴 但底下有个真缺口:所有共享知识按「话题」存,没有一样按 SKU 存。** 我手里拿的是 `c9e98cd7` / `77d8f781` 两个 id,**没有任何办法用 id 去查「这个 SKU 我们已经知道什么」** —— 是猜到 grep `op17` 才碰上的。**猜对了词才找得到,那是运气不是检索。**
- **⏳ 提议(等 Gary):`memory/SKU_FACTS.md`,按 `product_id` 索引,每条四样 —— 结论 · 谁定的 · 哪天 · 证据在哪。** 规矩两条:动一个 SKU 之前按 id 查它;定了一件事写回去。**把按日期的日志变成按东西的字典**,而我们所有的问题都是按东西来的。

## 🔴 9/6 归属定案:**Mystery Game 不是门店收入,是 ebay2 的**(Gary:「mystery 不是门店的收入 是ebay2的 / 是ebay系列盲盒的」)

- **这推翻了 9/1 那条的一半。** 9/1 定的「Mystery Game 从门店零售剔出」是**对的**,但当时只做了减法 —— **剔出去之后没有任何地方接住它**,于是这笔钱在报表上直接消失。**Gary 现在说清了它该落在哪:ebay2 的系列盲盒。**
- **量不小,而且三张表都有**:
  ```
  storefront_sales  215 行 / 1,133 件 / $42,290    (6月 $1,215 · 7月 $10,186 · 8月 $27,519 · 9月 $3,370)
  slabs             440 行            / $89,333
  singles            36 行            / $2,271
  ───────────────────────────────────────────────
  合计                                 $133,894
  ```
- **🔴 行上的 `location_id` 在说谎,别拿它做归属**:打了 Mystery 标的行按 location 记着 **Master $22,052 · Front Store $17,177**,而 ebay2 只有 $1,336。**那一列记的是货从哪个房拿的,不是卖在哪** —— 盲盒的货本来就是从 Master/门店抽出来装的。**按 location 归属会把 ebay2 的收入记成门店的,正是这条要修的错。**
- **⏳ 还没做(要动 `lv-finance` 侧,不是库存侧)**:`mystery_game.py` 现在只回答「是不是出奖」,**没有回答「算谁的」**。四个读者拿到 True 就整行丢掉。要做的是**从「剔除」改成「改记到 ebay2」**;在那之前 **ebay2 的营收是系统性低报的,而门店是干净的**。
- **⚠️ 和 9/1 那条不矛盾的部分仍然成立**:出奖行**不写收款**(`payment_method_id` 全 NULL、`net_cash_usd` 195 行全 NULL),所以现金链路不受影响;**「真收的钱」和「奖品价值」混在同一列**的问题也没变(7 笔均分 $6,390 是真钱 · 17 笔各异 $18,972 是奖品价 · 14 笔判不了 $5,685)。
- **🔴 而这个标签同时在反方向咬人:5 箱 Gem6 批发被打成 Mystery Game。** 08-19 起五笔,**每笔正好 20 盒=一箱、全部单行交易、全部从 Master 出**($1,000/$1,000/$720/$720/$720 = **$4,160**)。**真正的 Mystery Game 天生是多行的**(同期对照:4/6/9/12/13 行),因为出奖是一批奖品。**一笔单行、正好一整箱 = 批发。** 判据写下来:**行数 + 整箱量 + 出货房**,不是标签本身。
  - ⚠️ 其中 `08-25 qty 1 $720` 的价格和另外三箱一模一样,**几乎肯定是一箱录成了 1 盒 —— 19 盒库存从来没被扣**。没改(卖出行永不回改)。
  - 顺带:那五箱实际卖了 **$832/箱平均**(=$36–50/盒),而 Gary 记的是 $500/箱。**卖得比记忆里好** —— $500/箱对 $21.43 的成本只有 14% 毛利。

## ✅ 9/6 成本修了四条,零成本件数砍掉一半(Gary:「修成本」「20 per case gem6」「first partner 是要看那个盒子 有三个version」)

```
FB03 Raging Roar     $2.33/盒 → $140/盒(单价打进总额栏)     COGS +$8,260
Prismatic 散包        六行四个价 → 全部 $11.50               (9/2 就挂着等一句话)
Gem Pack Vol 6       $0 → $21.43 + 补一条从没入过账的进货     COGS +$8,572
First Partner Box    $1.66/$1.09 → $53/$49                  COGS +$3,799
────────────────────────────────────────────────────────────
零成本  501 → 257 件(−49%)· 账面 $223,754 → $227,154 · 活账补回 $20,631
```
- **Gem6 的单价一直算不出来,缺的不是钱是单位**:CN `入库*090` 记着 20 箱 × ¥2,880 = ¥57,600,**箱规没写**。Gary 给了 **20 盒/箱** → ¥144/盒 = **$21.43**(实时汇率 6.7194,不是 `supabase.js` 里写死的 7.14)。**三条独立检验只在 20 上同时成立**:比兄弟 Gem5($24.52)便宜 13% ✓ · 对我们自己 93 件均价 $49.81 是 **57% 毛利** ✓ · 之前猜的 40 盒/箱给出 $10.72,比 Gem5 便宜 56%,当时判「不合理」是对的。
- **First Partner 的三个 version 反而把答案给出来了**:同族四个 SKU,三个独立定价 **$43.20 / $44.87 / $45.53**,而争议那行的进货写着 **$1.66 / $1.09**。**没有任何 version 的盒子成本是一块钱。** 读成单价,$53 和 $49 正好落回它自己兄弟的带里。⏳ **仍未定:Front Store 那 10 盒是哪个 version** —— 若是 Series 2,该并到 `9c9a7cf7` 而不是留在笼统行上。
- **🔴 「卖出价低于成本」这个反证今天被推翻了**:早上因为三笔 $37.86/$43.75/$19.11 低于成本把 First Partner 按住,**而下午的门店审计证明那张表 45 行 / $2,878(窗口营收的 49%)是按件数均分出来的**,同一张表里还有个成本 $329 的盒「卖了 $53.33」。**低于成本的行价是分摊器在说话,不是货架。**
- **⚠️ Gem6 发票 400 盒,流向现在只差 21 盒**:在架 135 · 批发 5 箱 100 · 门店零售 12 · **直播房烧掉 132**(进 6 个房 170、还剩 26;**直播卖出不写销售行,这是正常的不是丢货**)。**无解释 21 盒全在 Master。**
- **⏳ 剩下 257 件零成本,最大两个不是查不到是数据本身坏的**:`Nivel Arena: Eterna Return` **69 件** —— 有进货行但 `cost_usd` 记的是 **$0.00**,而且 **JP 库存表 115 行、JP 入库表 1,805 行搜 nivel/nikke/eterna 全空**,`source_country` 还写着 USA。**产品是日文的(lang=JP 对),但这批货的钱在我们和日本两边的账上都不存在**,在架 69 件而进货行只有 24 件。要问人。

## ✅ 9/6 转库 10 行进 ebay2(Gary 口述,四轮问答定 SKU)

- **7 条 Transfer + 2 条无来源 Intake + 1 个新建 SKU。** ebay2 从 19 行/283 件 → **29 行/326 件**。
- **🔴 他说的源房和账不一致,三行都指向同一个方向**:`151 Poster Collection` 和 `Ascended Pin` 他说 Master、账上在 **Front Store**;两个 JP Mega 盒他说 PokeAuctionHouse、账上在 **Master**(PAH 两个都是 0)。**按账上真有货的房出货** —— 从 0 的房出货是凭空造货,而造出来的货会变成下一个人点货时的假缺口。**ebay2 那一端两种记法结果一样;差别说明的是「有货进过 Master/PAH 而没人记」。**
- **净增库存必须留痕**:两笔没有来源房的用 `movement_type='Intake'` + `from_location_id=NULL`(这张表已有的 5 条就是这个形状),**不走「直接改 inventory.quantity」那条零痕迹的老路** —— `inventory` 没有 audit log,裸改数量在构造上就查不出来。
- **⚠️ `movements.cost_basis` 存的是行总额不是单价**(旁边 Storm ×7 记 $768.60 而单价 ~$110)。第一版写了单价,7 条已改,脚本源码也改了。
- **幂等按 movement 判不按本地文件判**:脚本每轮从实时库存重建计划,没有这道闸门第二次跑会照着新库存**再搬一次**(数量乐观锁拦不住,因为锁也是从同一次新读里建的)。
- **新 SKU `d32efab8 Trick or Trade BOOster Bundle`**(建前扫全部 911 个产品确认零重复)。**名字里故意不写年份** —— 这货每年出,猜 2024 还是 2025 就是给货架贴一个自信的错标签。

## ✅ 9/6 上面那个 $22,842 已修:补层日期钉在切换点(Gary:「可以」)

- **`cost_layers.py` 加了一段补层**:单据日期早于切换点、但货是冻结之后才进库的,**给它建一层,日期写切换点不写单据日期** —— 它确实是从冻结之后才进这个模型的,写回单据日期会让 FIFO 顺序和真正更早的进货打架。**开账文件一个字没动,sha256 不变。**
- **🔴 自限的上限才是安全性所在,不是小心**:`补层量 = min(采购量, 在架 − 已有层量)`。**永远不可能补出比「没人解释的货」更多的层** —— 货本来就在照片里的那部分,缺口算出来是 0,自动补 0。**重复计在构造上不可能,而不是靠人记得。** 实测 **11 笔里 8 笔被上限咬住**(GEM4 采购 40 只补 26,另外 14 件本来就在照片里)。
```
修前   FIFO $181,691  账面 $204,533   差 -$22,842   OK 208 / 坏 13
修后   FIFO $198,954  账面 $199,295   差    -$341   OK 219 / 坏  3
补层 $13,439.36 + 余数 $1,541.44 = $14,980.80 = 那 11 笔原采购额,分毫不差
```
- **两张表都要报,而且余数要按比例摊**:第一版「仍未成层」那张表印的是**整行数量和整行金额**,而其中 26 件刚补过层 —— **「GEM4 qty 40 未成层」会被读成 40 件都没有层,再把那一列加起来就是错的**。已改成只印余数(qty 14 / $242.28)。**这正是这个文件本身要抓的那类错,只是这次指向它自己。**
- 看门狗 13 项测试全过。**零成本 498 → 529 不是变差** —— 那 13 个产品以前状态是坏的、根本不进统计,能算了才露出来。
- **⚠️ 新发现一个盲区,没修**:这次 FIFO 总额动了 $17k、坏产品 11 → 3,**而看门狗说「nothing changed that matters」**。它逐日比的是层和缺口;**开账文件有 sha256,算法本身没有** —— 改一行 `cost_layers.py` 的逻辑,所有 FIFO 数字会一起变而没有任何东西出声。今天是有意的,下次未必。

## 🔴 9/6 FIFO 对账面差 $22,842 的病根:冻结规则撞上「冻结之后才进库的旧单」

- **先说好消息,而且这是最要紧的一句**:**其余 208 个健康产品,FIFO 和账面只差 $336 / $181,691 = 0.2%。** 只要层模型关得掉,**两个口径基本是一致的** —— 冻结这套东西是对的。
- **$22,842 里 $22,506(98.5%)集中在 13 个产品上**,而这 13 个的 `fifo_value` 全是 **$0** —— 不是算错,是**根本算不出来**。
- **🔴 病根**:9/3 那 11 笔补录的进货**按规矩不成层**(日期早于 9/4 切换点,防止和开账里已有的货重复计)—— **但它们带进来的货是在 9/4 冻结快照之后才写进 `inventory` 的**,所以那批货**既不在开账层里、也没有任何新层**。逐个对得上,`Prismatic SPC 缺口 26 / 补录 26`、`SV Prismatic Bundle 18 / 18`、`Ascended Tin 6 / 6` **三条完全相等**。
- **⚠️ 我 9/5 说的「$15,001 是设计内、$7.8k 没解释」这个框架是错的。** 全部都是这 13 个产品;$14,981 只是那些补录的**采购价**,而账面背的是 **$22,506**,因为这些产品本来就有货、按加权平均计价。
- **🔴 不管它的代价是具体的:这 212 件卖出去会按 $0 计 COGS、报 100% 毛利**,而且**每补录一笔切换点前的进货,这个口袋就长大一次**。
- **📌 两个「FIFO 给不出成本」的口袋,而且不重叠**:`cost_known=false` 的 **498 件** + 这个缺口 **212 件 = 710 件**。报覆盖率要报 710 不是 498。
- **⏳ 处方(要 Gary 定,我没动)**:给这类货补**一笔日期钉在切换点的调整层**,标明是补录来源。**开账文件绝对不动** —— 它一改,自冻结以来每一个 COGS 都被追溯改写,而看门狗每天验的就是它的 sha256。这是改 COGS 算法,不是清理。
- 另有 2 个对不上补录的:`Ascended Heroes Booster Bundle` 缺口 3(小差)· `[EN] OP-11 … Booster Box` NO_LAYERS 5 件(另一个产品行,单独查)。


## ✅ 9/5 双轨日卡上线 + 信箱协作(财务线 MSG-006,Gary 会议室原话「两边同时推进给我结果」)

- **协作走文件不走聊天**:`~/.claude/projects/c--Users-Gary-Desktop-LV-Agents/memory/AGENT_MAILBOX.md`,三条会话共享,**只追加,唯一允许改旧条目的字段是状态**。`SendMessage` 退化成门铃。⚠️ **`lv-agents-e9` 不在我的 `ListAgents` 里,发不过去** —— 所以信箱是唯一可靠通道。
- **✅ 新 `inventory_day_card.py` + 任务 `LV Inventory Day Card`(每天 20:10)已上线实跑通过**(exit 0,卡已进信箱)。**20:10 是排在 19:13 房间对账之后** —— 早于它跑,卡上的消耗数是对账还没算出来的那个,两张卡会为同一天打架。**只读:不动库存、不写成本、不对外发。**
- **🔴 而财务那份契约里有一条会给出自信的错数,已当场改掉**:他们提的消耗口径是 `actual_qty(t2) − actual_qty(t1)`。三个原因不成立 —— ① **ledger 房(Front/Master)的负差是无解释短缺不是消耗**(8/24 为这条改了 8 个消费方)② **正差从 7/24 起不写库存**,所以两次 `actual` 之间夹着一个从没被记录的修正,相减是拿两个不同基准做差 ③ **归零的行会从盘点表消失**(`.gt('quantity',0)`),下一次的「差」是凭空造的。**已改成直接调 `count_sales_recon.build(collect=)`** —— 复用发版在跑的那台机器,日卡和日巡永远不会打架。
- **卡上写死三件事**:**业务日印出来**(我 20:10 覆盖当天、财务 09:43 覆盖昨天,不写明就对不上;`created_at` 是 UTC,20:02Z 是 13:02 PT)· **`N/C` 单独成桶不许压成数字**(今天三个房两个 N/C,压成 0 就是把「没查」渲染成「没异常」)· **看门狗读各自日志的 exit 行不读调度器状态**,读不出来印 `?` 不印绿(任务 exit 0 也可能什么都没做 —— `jp_shipment_watch` 的 `--telegram` 被 except 吞了好几周)。
- **首卡实况**:FIFO 第 0 层 **93.4%**(前一天 99.3%,**在长**)· FIFO $181,691 vs 账面 $204,533 **差 −$22,842**(其中 $15,001 是 11 笔补录的切换点前进货,设计上不成层)· **零成本 498 件** · `OVER_ON_HAND 11 + NO_LAYERS 2` · Packheads N/C burn 705 多出 110 · RocketsHQ N/C 多出 55。
- **⚠️ 多出永不写库,所以两轨必然有一个结构性固定差**(今天 110+55)。**那不是任何一方错了**,建议对照卡列成具名行,否则每天都有人去查一个查不出结果的差。

## 🔴 9/5 并发写同一个文件:断言救了一次,判据太粗也记下来

- 往信箱写回复时,**财务线在我读和写之间追加了 MSG-004** —— 我如果用旧副本写回去,会**把他们那条整个抹掉**。断言拦住了。
- **但第一版断言是「全局 DONE 计数 == 2」,那是被别人的并发写入触发的,不是被我的错误触发的** —— 判据太粗会把正常的并发报成异常。已改成**按条目头定位、只改目标那一行的状态字段**,并在写之前重读。**共享文件一律:写前重读 + 只改自己那一段 + 断言别人的条目还在。**

## 📌 9/5 财务线交来的数据质量队列:155 条里 84% 不该给人看

- **`duplicate_identity` 60 组 → 给人的 0 条。** 他们的 identity 是从名字生成的 slug,不知道「Pokemon Center Elite Trainer Box」整个是产品名、**套名才是区分符** —— `[box-center-elite|box|EN]` 一组塞了 7 个不同的套。**和我们 8/19 那个匹配器踩的是同一个坑**(改了五版才从 50.4% 压到 3.2%,病根是「包装词和版本词看起来像噪音,其实就是产品」)。
  - 用**发版那个匹配器**在组内逐对跑:**60 组只认出 3 对真重复,全部零库存**;而且**其中 2 对早在 8/19 就合并过了**(`active=False` 已经在那儿)。真正还开着的只有 1 对。
  - **⚠️ 我第一版验证脚本报「58/60 是误报」,那个数是假的** —— 控制组写成「匹配器必须响」,而这个守卫本来就是调成 **0.9%**(全 874 个在售产品实测)才响的。**是控制组错了不是匹配器错了。** 这类过滤必须带对照组跑。
- **`uncosted_sales` 60 条 → 给人的 0 条。** 只有 **2 个 listing**(`$ RTYH lfg!!!` 58 行),`sku` 是 `5/6/7/11/261` —— **拍卖坑位号**。**没有人能说出 261 号坑里是哪张卡。** 这是 **1 个口径决定**不是 60 条清单。
- **`unit_price_outlier` 35 条 → 这段是真的**,剔掉 3 条拿零成本层当参照的(`Celebrations 单价 $34 vs 其它层 $0.14`),**28 条真新的**,而且是 **4 类病**:①单价当总额(大数量、单价<$1,几乎全在 6/24 开关之前)②qty=1 的行背着另一笔的行总额(`Surging Sparks Bundle 同一天 x1 $52.70 / x15 $52`)③Case 按盒计价 ④**`[EN] OP-06` 同产品同数量两笔差 10 倍($29.17 vs $287)**。**按类问 4 个问题,比按行问 28 次更可能得到答案。**
- **契约:过滤在库存侧,不在文件里。** 财务出候选 → 库存过匹配器 + 对已知在办事项 → 剩下的才进人的清单。**不加这步 delta 制永远收敛不了** —— 手册里这个坑踩过两次(已拍板的决定被反复要求再决定,把真信号埋了 36 天),而这次队列**第一天就报了两条已完成的工作**。

### 🔴 顺带查实的两条
- **`FB03 Raging Roar`**:6/13 Frank 从 Discord 60 盒 **$140 = $2.33/盒**(同类 $73–$714),在架 21 件(PK 18 + Front 3)。**而 8/05 已经卖掉一件 $295、`cost_basis` 记 $2.33、报了 99.2% 毛利** —— 假毛利已经在历史账里。**要 Frank 说真单价,确认前不猜。**
- **`SB02 Manga Booster 02`**(8/19 Frank 8 盒 $5,712 = $714/盒):**inventory 侧查不到任何一笔销售**(storefront / platform_sales / online_order_items / movements 全 0)。而**三场盘点(9/4 Polar ×2、9/5 Juney)都在 RocketsHQ 数到 12 件、账面是 0**,8 件挂在 Master;**12 ≠ 8,我们只买过 8 件,多的 4 件没有来路**。RocketsHQ 那行 basis $0.00,从那边卖出去会报 100% 毛利。(9/2 Juney 把 Master 数成 0 那场是 `deleted=True` 已撤回,Master 的 8 件没被擦。)
- **⏳ 两条等 Gary**:那 1 对零库存重复合不合 · 坑位销售($2,135)成本口径。**派单里写「Gary 已批」,但平行会话不能替他批我这边挂起的写库** —— 零库存所以不流血,等他一个字。


## ✅ 9/4 夜 门店买入「从今天起看」已上线(Gary:「没事 从现在开始」)—— 而查出来的历史缺口是 91%

- **起点是一个小问题**:Gary 问「他这个付的 178.19 你是哪里得出来的」。**答案是没有人付过这个数** —— 店里那行写的是 `1 darkness ablaze etb - $110`,$178.19 是分摊算出来的。
- **🔴 而 14 行全部被扔掉了**。解析器把行尾金额抓出来存进**备注文字**(`listed $110`)却没存成**数字**,于是分摊算法看到「14 行都没价」,按 TCG 市价权重摊 —— 而 TCG 只认得其中 6 行,**那 6 行把 $2,370 全吃了**。最离谱两条:`Mega Charizard UPC` 店里写 $240 **记成 $71.80**;`FPS3 ×10` 写 $300 **记成 $718.18**。**$1,371(58%)记在错的行上,而总额一分不差 —— 所以任何金额级对账都抓不到。**
- **✅ 已修**:`parseBuyList` 新增数字字段 `listed`,分摊权重改成 **店里写的 → TCG 市价 → 平均**。**故意仍不当实付价** —— 9/01 和 9/04 那两张写的是市价、9/03 那张写的是实付,**文本里分不出来**;但**当权重两种读法下都成立**,而且覆盖 14/14。测试 14 项跑发版源码,**其中两条把旧行为钉死**:去掉数字后算出 $718.18 / $71.80,**和生产库逐分吻合**,证明测试架子在跑真代码。
- **✅ 「这些是市价」已用 TCG 逐行验死(Gary 判对了)**:`$33/32.99` · `$195/194.35` · `$155/152.95` · `$150/150.19` · `$66/66.88` · `$240/234.36` —— **14 行全部落在市价 1–4% 以内,不可能是巧合。**

### 🔴 顺着群聊查下去:8/13–8/31 门店买入 91% 不在系统里
```
日期        群里说付了   收银台   buy-list        8/13–8/31 合计
08-20         10,334     1,400        0          群里 $38,444
08-28         10,710       990        0          系统  $3,472   ← 91% 没有
08-26          6,055         0        0
08-31          5,642         0        0
```
- **9 月开始有数,是因为 9/1 之后 Mario/Aldo/Sully 被专门交代过** —— 不是系统好了,是那三笔被人盯着做了。另有 **8 笔完全定不了日期**($14,020),含 `Trade in $7500`。
- **最刺眼的一笔是你自己问过的**:Sully `Trade in $7500 / Trade out for sealed today / Sorry forgot to post` → **Mr. Vault「do we scan out in system / if not just drop certs and also sealed list please so my system can update skus and record」** → `we will figure tomorrow`。**「明天再弄」之后系统里至今没有。**
- **⚠️ 纠正手册自己**:旧条目写「新建单卡 365 张,有成本的 0 张」**已过时** —— `singles_intake_batch.py` 现在会写成本(8/13 起 384/435 行有成本 $16,080)。**但写的是「市价×80%」的推算值,不是抽屉里出去的钱。** 所以形状是:**货进来了 · 成本是估的 · 现金那一侧没有记录** —— 这正是「门店说没有 cash」查不清的根。
- **逐笔核过所有「金额碰巧对得上」的 acquisitions,全是 Frank 的 Discord 单 / HUA 的日本单 / CN Base,没有一笔是柜台的。**

### ✅ `chat_buy_sweep.py` + 任务 `LV Chat Buy Sweep`(每天 12:10)已上线实跑通过
- **切换点 `CUTOVER = "2026-09-04"` 写死在代码里** —— 照 GTS(8/12)和日本(8/13)那两次的规矩,**写在名单里的政策会忘,写成规则的不会**。`--all` 仍看得到历史。
- **12:10 是特意排在 `LV_Lark_Chat_Refresh`(11:45)之后** —— 反过来会读昨天的快照、天天报平安,**一条构造上永远不可能响的看门狗**。
- **快照超过 36 小时就报「查不了」不报平安** —— 输入死了却安静下来的看门狗比没有更糟。
- **只报没报过的,而且等那一天过完才报**(`GRACE_DAYS=1`)—— Aldo 的 9/03 买入是 9/04 早上录的,**当天就催会教会人忽略它**。
- **三个分支实测**:切换点拨回 8/26 → **当场报 29 条**(最大 `We buy from Michael $8100`)· 再跑一次 → **0 条不重复** · 快照调成过期 → **明说「could NOT be made」**。**只读,一个字不写库** —— 群里的金额永远不能直接改库存。
- **⏳ 但看门狗只能告诉你漏了,补不回来。真正的修法是让门店走 `/buy-list`** —— 9/1 之后三笔($6,105 / $11,198 / $2,370)证明这条路能用。

## 🔴 9/4 夜 销售看门狗把 trade 的均分当成真销售在报(已修)—— 我昨天刚上线的

- 今天日报那句 **`median 273% of market` 整个是假的**:9/03 那笔 vendor trade 出去 16 件,app 按「even trade」**把 $2,370 均分成每件 $148.13**,一张 $2 的 Shinx 和一张 $200 的 Raikou ex 记成同一个价。**7 件有市价的 × $148.13 ÷ $379.68 = 273%,分毫不差。**
- **trade 的行上没有「价」这个东西** —— 那是一个分摊,不是任何人对那张卡同意的价格。已加排除,**并且明写排除了几笔**(静默丢桶会读成「本来就没有」)。
- **修完当场浮出两笔真的低价**:`09-04 $75 卖在 48% of market(Mega Feraligatr ex)` · `09-02 $60 卖在 78%`。**噪音盖住的正是真信号。**

## 📌 9/4 夜 `market` vs `median`:median 是一条没人成交的要价,不能当锚(Gary:「市价不对」)

- TCG 的 `pricepoints` 返回两个数,我们叫 `market` / `median`。拿 Destined Rivals ETB 实测:
  ```
  当前在售 154 条,最便宜 20 条要价   $42.39 … $118.75 … $124.99
  最近 5 笔真实成交                  $118.75 $118.74 $118.54 $111.00 $79.89
  marketPrice        $118.18   ← 和成交完全重合
  listedMedianPrice  $139.71   ← 比这 5 笔里的每一笔都高
  ```
- **买家从队头买,median 量的是整条队伍的中点**,被上面那条贵尾巴拉高。全库 192 个两个数都有的产品:**96% 是 median 更高,中位高 12%**。
- **所以「$112 不对」这条查下来是数对的**:Darkness Ablaze ETB 最近成交 $105.97–112.98(中位 $112.98),我们 feed 记 $110.99,**而且它是模糊匹配、匹配对了**(`216856`)。
- **我一度建议「按 median 报或者两个都报」,是错的,已撤回。** median 只发进了 feed 当诊断用,**没有接进任何通知,所以没有任何百分比因此变过**。

## 📌 9/4 夜 9/03 那笔 vendor trade 两侧都定完价了(Gary:「我们 trade 了 90% 拿下了 sealed 产品」)
```
进来  14 件密封,逐行核过 TCG           市价 $2,082
出去  16 件(5 密封 + 11 单卡)         成本 $728.51 · 市价 $1,216.56
```
- **拿 $729 成本 / $1,217 市价的货,换回 $2,082 市价的货 —— 这笔赚了。**
- **但账上写成相反的样子**:出去那侧记 $2,370 收入 / COGS $728.51 → **9/3 记了 $1,641 利润**;进来那侧记 $2,370 成本,而市价只有 $2,082 → **以后卖掉会报 $288 亏损**。**总数是对的($1,641−$288=$1,353),时点和落点是错的。**
- **⏳ 要定的口径**:trade 拿回来的货,`acquisitions.cost_usd` 该记什么?现在记的是**给客人的信用额**,那既不是我们付的也不是货值。**A 记这批货自己的市价 $2,082**(可核对)/ **B 记给出去那批货的成本 $728.51**(最保守,但**输入本身是坏的** —— Mimikyu 成本 $0、两件单卡没成本)。**建议 A。** 不管选哪个,**出去那侧的销售行不碰**(卖出行永不回改)。
- ⚠️ **群里 Sully 写的 `Market $2800`,而 14 行逐行核下来只有 $2,082,差 $718,没解释。**

## ✅ 9/4 晚 单价打进总额栏:38 条已修,**COGS 补回 $89,039.70**(Gary:「我们应该只改单价 总价是自动计算的」)

- **Gary 纠正了我的方法,而他是对的**:我原来打算拿 `avg_cost_basis` 反推一个新总额 —— **那是个加权平均,等于用一个猜的数去改另一个数**。正确的读法简单得多:**字段里那个数就是有人打的单价,总额 = 单价 × 数量。** 和 8/20 Journey Together 那次 `FIXED_UNIT_AS_TOTAL` 是同一招(400 包记 $5.50 → $2,200)。
- **✅ 已写 38 条**(`fix_unit_as_total_0904.py`,备份 `unit_as_total_backup_0904.json`,**乐观锁钉旧 `cost_usd` + 逐条回读 + notes 打 `FIXED_UNIT_AS_TOTAL`**):`$2,401.15 → $91,440.85`。最大几条:Prismatic ETB 108 个 $135→$14,580 · 151 散包 400 个 $25→$10,000 · PRB-02 30 盒 $315→$9,450。**独立回读 38/38 全对。**
- **只动 `acquisitions`,`inventory` 一个字没碰** —— 而且这 38 条全在 9/4 切换点之前,**在冻结的开账层里,永远不会成为 FIFO 层**,所以账面价值和成本层台账都不受影响。**变的是历史 COGS 和毛利,那正是错的那个。**
- **1 条按住没写**:`f7ca2fa5 White Flare` 是 **JPY** —— `cost` 是日元、`cost_usd` 是美元,修两列要当时录入用的汇率,而行上没有。**宁可留着也不猜汇率。**

## 🔴 9/4 晚 Mini-tin:我上一条报反了,而且真问题是重复入账(Gary:「看 frank 的采购价格」)

- **我说「账面比实付低了约 $7,417」是错的。** 那个 +$7,417 是 FIFO 算出来的,**而它是被一条重复的进货撑出来的**,不是账面偏低的证据。**看到分歧先查层,别直接读成账面错。**
- **Frank 自己的原话(`member_inbox.jsonl`)**:「I am buying 3 tin sets of ts03 asia exclusive one piece tins for **a total of 8900**」,之后又三次记成「Total - $8900 … **[Set of 6] Tin (16)** - $8900」。
- **🔴 库里是同一笔 $8,900 记了两遍,只是单位不同**:`08-19 qty=96 @ $8,899.20`(按罐)和 `08-19 qty=16 @ $8,900.00`(按盒,16×6=96 罐)。**钱一模一样、日期一模一样、买手都是 Frank —— $8,900 双计。**
- **⚠️ 但「49 件」到底是罐还是盒,数据自己答不了**:产品名写着 `[Set of 6] Tin`(那 49 就是盒,可我们只买过 16 盒);按罐算 96−49=47 消耗,和手册 9/1 记的「47 罐」对得上。**两种读法各有一半证据 —— 这正是单位铁律那一列不存在的代价。** 另有一条 `09-01 qty=2 @ $2,226`($1,113/件)来路不明。
- **⏳ 一句话问 Frank 就能定:$8,900 买的是 96 罐还是 16 盒(每盒 6 罐)?** 定了之后:删掉重复那条 + 开账层按真实单价补一笔调整层。**在那之前一个字不写。**

## ✅ 9/4 单卡没市价的真病根:**入库那一步一直在做,是后面克隆时把结果丢了**(Gary:「入库的时候不是有 singles 查询入库的状态吗 那个就是一个 cost base analysis 对的」)

- **Gary 说对了,而我前一条判错了。** 我原来写「app 里建的卡没人定价」,像是缺一套定价机制。**其实机制一直在跑** —— 批次入库工具查价、算成本(市价 × 80%)、写表、小时级同步。**丢价的是克隆。**
  ```
  190 张没有市价
    128 张来自「卖掉一叠里的一部分」的拆分行
     35 张来自「柜台已售恢复」
    ────────────────────────────────────
    163 / 190 = 86%  是这两条克隆路径丢的
  ```
- **🔴 代码里三个克隆点,只有一个复制了市价**:`Move Inventory` 的拆分**一直有**;`sellSingleQtySplit`(卖出拆分)和 `_recoverSoldSingle`(柜台恢复)**都漏了**。数据完全对得上 —— **156 个拆分行里 128 个空,而有价的那 28 个正是走 Move Inventory 那条**。
- **两处都复制了 `tcg_id`(卡的身份)却漏了市价,所以这是遗漏不是决定。** 恢复那条**故意**把成本和来源留空是对的(手上有这张卡,不证明它来自同一笔进货,抄成本会把同一笔 COGS 记两遍);**但市价不是来源信息,它是「这张卡值多少」,而上一行的 `tcg_id` 已经说了是同一张卡。** 已补,并把这个区别写进注释。
- **⏳ 历史那 163 行我没有回填,而且这是个决定不是遗漏**:162 行能从同 `tcg_id` 的兄弟行取到价,**但只有 1 行还在架上,其余全是已售出的行**。给一张 8 月卖掉的卡填上 9 月的市价,会让「当时卖了市价的百分之几」变成拿八月的成交价比九月的行情 —— **正是 8/10 记过的「拿今天的市价比当时的买入,量的是市场不是买手」**。所以**只修向前**,看门狗里那 33 笔「判不了」会自己减少。

## ✅ 9/4 已发版 `0d09aa4` + sealed 也报市价 + 销售看门狗上线(Gary:「下一个发吧 然后 sealed 也可以发 发 market 是多少 但是这个 require 我们做 watchdog 去分析处理」)

- **✅ 发了四个文件**(解析器 / buy-list 付款+singles lot / 销售通知带市价 / % of market)。**Gary 说门店那份教程他已经发出去了 —— 而解析修复当时还没上线,所以这次是团队正在撞的东西,先推为敬。** 全套闸门过了才推:解析器 118 项 + % 6 项 + 两轮变异 + 874 个名字双版本对跑(qty/名字/top-1 全部 0 变化)+ build。
- **✅ sealed 也报市价了,但只认钉过价的**(`marketFor(...).pinned`)。**fuzzy 匹配不算** —— 今早那个 `AH TIN 186% of market` 就是 fuzzy 命中了另一款 tin 的价格,**模糊匹配是证据不是身份**。没钉价的密封品算「未定价」,进覆盖率那一句,不去污染百分比。
- **✅ 新 `sale_market_watch.py` + 任务 `LV Sale Market Watch`(每天 09:50)已上线并实跑通过**(exit 0,**Telegram 真发出去了**)。**09:50 是特意排在小时级 sheet sync 之后** —— 早于它跑,会把「马上就要拿到价」的卡数成未定价,而覆盖率正是这个看门狗的立身之本。
- **它盯三件事,而第三件是前两件会不会说谎的原因**:① **卖得低于市价**(逐笔点名,不报日均 —— 日均没人能行动)② **覆盖率**(低于 50% 直接不给判定,而不是印一个带免责声明的数)③ **定价漏洞本身**(app 里建的卡 `source='manual'`,没有任何东西给它定价;**一个看门狗如果只看百分比、不看漏洞,就会在数字失去意义的那一刻正好安静下来**)。
- **首跑 30 天实况**:153 笔单卡交易,**能判的中位数 106% of market**(柜台大致按市价卖)· **33 笔判不了** · **7 笔低于 80%**,最重的 `08-11 $195 卖在 49% of market(Mewtwo、Erika's Dragonair)`。⚠️ 但 `Erika's Dragonair` 的市价本身就是零成交定出来的(手册 8/12 记过),**百分比只能和它的市价一样可靠**。
- **首跑当天就抓到活的**:「最近 1 天新建的 13 张卡,市价全空」。

## 🔴 9/4 「为什么全是 $28」+「能不能报 % of mp」(Gary 问两笔门店单卡销售)

- **$28 不是谁定的价,是 $280 ÷ 10。** 收银员输的是整车总额,app 按参考价加权分摊,**而那 10 张里有 3 张完全没有市价**,加权算不了 → 退回均分。同一天另一笔 $95/6 张分出 $40.15 / $23.75 / $3.39 各不相同,那是加权成功的样子。**总额对、收款对,逐行价格是均分的产物 —— 一张 $4 的草能量和一张 $14.46 的 Espeon GX 都记成卖了 $28。**
- **🔴 而「为什么没有市价」是系统的洞,按创建月份看在加速**:
  ```
  2026-05  891 张   0% 空
  2026-06 1082 张   3%
  2026-07  614 张   8%
  2026-08  497 张  18%
  2026-09   23 张 100%   ← 九月建的每一张都没有市价
  ```
  **机制**:走 Cards Scan → 表格 → 小时级 sync 的卡拿到 `market_price_source=sheet_sync` 和价格;**在 app 里直接建的卡(柜台恢复 / 手工加)是 `manual`,没有任何东西负责给它定价**。8/10 停掉的 `LV_Singles_Daily_Price_Refresh` 本来是唯一会回头补的那道网。**196 行没有市价,而 196 行全都有 tcg_id —— 价格取得到,只是没人去取。**
- **✅ 已补 8 张在架的(`--blank-only`,备份 + 回读 8/8)。新开关只填 NULL、一个已有价格都不碰** —— **这正是 8/10 那次没敢跑的区别**:那次要改 591 个已有价格、大面积 +200~300% 说不清;**填空不可能毁掉一个数,因为那里本来没有数。**
- **⚠️ 闸门当场拒了 9 张,拒得对**:Normal 无人挂牌、只有闪版价 —— `Slowpoke $137.46` · `Ponyta $112.72` 明显是闪卡价,而我们手上多半是普卡。**拿闪版价填,$9 的卡会变成 $122**(8/10 记过)。这 9 张要人说清手上是哪个印次,**系统没记印次**才是根。
- **✅ 销售通知加了 `sold at N% of market`(Gary:「我们可以做一个」)——未发版**。
  - **🔴 而这个数我第一次手算就算错了**:拿全部 $280 去比只有 7 张卡的市价 $88.44,得出 **317%**;**分子 10 件、分母 7 件**。**像对像**地只算有价的 7 件是 **222%**。已写成代码并**把 317 这个错法做成变异测试** —— 把分子换回整车总额,测试当场红。
  - **覆盖率跟着数走**:不足全覆盖时印 `(7 of 10 priced)`;**一张都没有价就明说「% not checked」,绝不当成 0**。买入不印这一行(我们在付钱,不是在卖)。
  - 测试 6 项 + 变异 2 红,`npx vite build` 通过。

## ✅ 9/4 `/buy-list` 加付款 + singles lot(Gary:「记一下付款 然后 singles lot 也记一下 然后按照 batch 记 batch入库」)—— **未发版**

- **📌 三件事里 batch 已经在做了,查了才知道**:`acquisitions.batch_id` **这一列早就存在**(342 行 / 151 个 batch,5/28 一直写到今天),`BuyListIntake` 每次提交就生成一个,`IntakeToMaster` 也已经按 batch 分组收货。**所以「按 batch 入库」不用做,只要别把它弄丢。** ⚠️ 那 151 个 batch 至今是**孤儿** —— `batches` 表的 DDL(`add_batches_and_channel_map_2026_08_16.sql`)还卡在 Supabase 权限上,所以 batch 有 id 但没有名字、没有总额、没有人能描述它。
- **✅ 付款**:新增必填的 **Paid with**,写进每一条 acquisition 的 `payment_method_id`。**「还没付」用现成的 `IOU (we owe)` 而不是留空** —— 留空和「没人填」分不出来,而那正是 500 行没有付款方式的来路。
- **🔴 而且只有现金才写收银行**:选 Cash 时额外写一条 `transaction_type='buy'` / `net_cash_usd = −总额` 的门店行,抽屉才对得上;**Zelle / 转账 / 刷卡不写** —— 把它们记成现金流出,下次盘点就会正好短这个数,**而「把正常收班报成短缺」正是 7/25 之后六周没人盘的原因**。
- **✅ singles lot**:可选的「几张卡 / 付了多少」,**只记钱和张数,绝不写库存** —— 卡由 Cards Scan 逐张入库,这里再记一次就是每张卡算两遍。形状和收银台的 Bulk Buy 一致(`product_id` 为空的 buy 行),所以 Cards Scan 只需要看一个地方。
- 落点默认本来就是 **Front Store**,Gary 那条「入库地点就是门店、不转 master」不用改代码。
- `npx vite build` 通过,页面实拍确认三处都渲染正常。**⏳ 和解析器那两处改动一起,`src/` 全部未提交未发版,等一句话 + Codex。**

## 🔴 9/4 「直接让他们输入 item list 就行」—— 一试,门店真实写法**数量 0/7 全解析不出**(已修,**未发版**)

- Gary 定的方案是让门店粘 item list(不扫码)。**发教程之前先拿他今天贴的真单子跑了一遍,幸好跑了**:
  ```
  ✅8x AH MEGA EX BOX - $48     →  qty=null   name="✅8x AH MEGA EX BOX"
  ```
  **7 行 0 个数量。** 价格倒是 7/7 都进了 note。
- **两个病因**:① **每行开头那个 ✅** —— 店员逐项打勾用的,它和数字粘在一起,任何数量规则都不匹配,而且**这个符号还跟着进了产品名去参与匹配**,三行因此一个候选都给不出 ② **`8x NAME` 这种写法没规则** —— 只支持结尾的 `xN`、结尾的 ` - N`、和开头的裸整数。
- **✅ 已修两条**:开头装饰剥离 + **`^(\d+)\s*[xX]\s+` 当作显式数量**(排在裸整数之前)。`x` 后面必须有空格,所以 `2x2 Ultra Pro` 不会被读成 2。**修完 7/7 数量、7/7 价格、6/7 首选正确。**
- **⚠️ 我第一版的剥离规则写成「非字母数字一律剥」,把我们自己 `[JP]` 的方括号吃掉了 —— 874 个名字里 177 个变成 `JP] …`。** 而且**什么都没坏**(分词器本来就丢括号),`top-1 移动 0 个` —— **正是这种「今天恰好无害的静默损坏」最容易过评审**。已改成只剥真装饰(空白/项目符号/破折号/箭头/emoji),**括号和 `$` 是结构不是装饰**。
- **✅ 双版本对跑(手册规矩)**:同时 import HEAD 和工作区两个版本,**874 个在售名字全跑** —— **qty 变化 0 · 解析名变化 0 · top-1 移动 0 · 自己找得到自己 781→781**。**这个改动对现有目录完全惰性。** 15 条对抗输入只有 3 条变,全是变好的(`✅8x…` null→8 · `- 5 Journey Together` null→5 · `30 X Mega Charizard` 名字清干净)。
- **`151 booster bundle` 仍然读成 151 件,是故意没动的**,并且**写进测试钉住** —— 那是另一个已知问题(151 是套名),以后要改必须是个决定,不能是这次的副作用。
- 顺带补了 5 个门店自己的缩写(`bbox` / `bbundle` / `pris` / `fps2` / `fps3`)——**这三行之前是一个候选都没有**,不是排错而是空的。**只收一个缩写对一个产品族、没有歧义的**。
- 测试 105 → **118 项全过**,并做了变异检查:**关掉 leading-`Nx` 那条 → 当场 4 条红**。`npx vite build` 通过。
- **⏳ `src/lib/buyListParse.js` 的改动还在工作区,没提交没发版** —— 按规矩等你一句话,顺便过一轮 Codex。
- **⚠️ 还有一条 `/buy-list` 的硬伤要一起想**:它**完全不记付款**(整个文件没有一处 `payment_method`)。所以「粘 item list」这条路记得了货和成本,**记不了钱** —— 昨天那 $11,198 就是这样,货和成本今天补上了,现金那一侧仍然是空的。**如果这批以后付的是抽屉现金,还得单独补一条,否则抽屉又对不上。**

## ✅ 9/4 门店 9/3 那笔买入 $11,198 已补成本(Gary:「给他们补一下cost吧」)—— 又是「手工加 Master」那条零痕迹的路

- **Gary 先让我查是不是 Aldo 已经做了(「我感觉已经有 log 了」)—— 查了,直觉对了一半。** 确实有动作:**`2026-09-04T15:36:35–37`,六行 inventory 在两秒内被写,数量和单子分毫不差**。但 **movements 0 条、receipts 0 条(那 8 条全是 Frank 的 OP17/OP14 和 HUA 的日本货)、acquisitions 0 条** —— 这是 `/manual-inventory`,**它只改数字**。而 `inventory` 没有 audit log(DDL 从 7/23 积压至今),**所以做这件事的人在构造上查不出来**。Aldo 18:48–19:15 的动作是**把货从 Master 转出去**,不是入库。**先手工加 Master、再转出来 —— 和 9/01 一模一样。**
- **🔴 单子上的数字是「实付」不是市价,这个先验了才敢按成本入账**:四条能核的落在 **76–83%(加权 82%)**,而 **9/01 那张是市价(98–101%),我们付它的 80%**。**两张单子长得像,含义相反 —— 直接按市价入账会把 COGS 记高两成。** ⚠️ `AH TIN` 那条 186% 是 fuzzy 匹配噪音(我们有好几款 Ascended Heroes tin),不当数。
- **✅ 已建 7 条 acquisitions = $11,198,basis 按加权平均重算,`quantity` 一个字没动**(货已在架,补货就是翻倍)。备份 `storebuy_0903_backup.json`,乐观锁 + 逐条回读 7/7,自己盖 `last_updated`。
- **「还没付」(Gary)所以:不写付款方式、不写现金行、notes 打 `UNPAID`。** ⚠️ **`acquisitions` 没有付款状态字段**(8/07 记过,那笔 $8,335 付 $4,000 欠 $4,335 系统看不见)—— **所以这笔应付只活在 notes 里,任何报表都看不见它。**
- **⚠️ 两行被零成本老货严重稀释,写之前就报了数**:`FPS3` 架上原有 2 件无成本 → 新 basis **$24.04 而不是 $25**;`AH TIN` 原有 **21 件**无成本 → 新 basis **$8.89 而不是 $40**。**总金额是对的,单价被无主的 0 拉低** —— 和 9/01 Pitch Black 同型。
- 顺带:`DR ETB` 原来挂着 **$140**,而这批实付 **$90** —— 零库存时期留下的旧值,已随加权平均纠正。
- **📌 而正确的路一直都在,只是没人用**:`_buySealedLine` 做的正是 Gary 要的 —— **`updateInventory(product, frontStoreId, qty, price)` 直接进门店、按加权平均算成本,同时写一条 `transaction_type='buy'` / `net_cash_usd = −总额` 的收银行。一次动作,货、成本、现金、地点全对,而且完全不碰 Master。**

## 🔴 9/4 「门店说没有 cash 对吗」—— 很可能是对的,而且我们没有能力反驳(Gary 问)

- **系统说抽屉里应该有 $10,846.17**(以 7/25 最后一次实盘 $457 起算,+ 现金销售 $10,941.50 − 现金买入 $552.33)。**但这个数只记了钱进,没记钱出,所以它不是证据。**
- **🔴 柜台买入的记录已经塌了**:`storefront_sales` 按月的 buy 笔数 —— **6 月 8 · 7 月 36 · 8 月 3 · 9 月 0**,而同期 sale 一直正常(8 月 242 · 9 月 43)。**买入并没有停,停的是记录。** 已知却没进系统的现金流出:9/01 那单买取 **「$3,000 from hard box」**(手册记的,总额 $6,104.77)· Gary 自己 9/01 口述的 **買取支出 $1,175** · 8 月群里 ~**$3,120**(手册 9/01 记的 6 笔)—— **光这三项 ≈ $7,300,已经吃掉 $10,846 的大部分**,还没算日常收班把现金拿走、递给 Mario/Frank 去买货(账本备注里全是:「Mario took $1400 for buys」「1k handed to Mario」「gave Frank $202」「cash taken off-site」)。
- **🔴 而最要命的是:41 天没有人盘过现金。** app 的权威表 `cash_counts` **共 40 行,最后一行停在 2026-07-25**,和 `cash_ledger.json` 的 `last_count_date` 完全一致 —— **两个独立来源互证,不是某个文件坏了。**
- **两个提醒的状态解释了这个盲区是怎么长出来的,而且顺序和直觉相反**:
  - **`LV_Storefront_Cash_Reminder_AM` / `_PM` 两个任务现在是 Disabled**,日志跑到 **8/09** 为止。
  - 但**盘点是 7/25 就停的,提醒又空转催了两周才被关掉** —— **所以关提醒不是原因,是反应。真正的顺序是:门店先不盘了 → 提醒天天喊没人理 → 有人把它关了 → 从此没人再提。**
  - **`LV_Eric_Cash_Reminder`(周一三五 09:30 发 LV MAG)今天 9:30 还在正常发、`posted: True`** —— **它一直在响,只是响给一个没人接的地方**,而且没有任何东西把回复捞回 `cash_counts`(`cash_count_ingest.py` 存在但**没有任何计划任务**,最后修改 7/21)。
- **📌 所以诚实的结论是:「有没有少钱」现在答不了,而且不是数据不够,是唯一能答的那个动作(实盘)41 天没做。** $10,846 和「抽屉是空的」都成立 —— 前者是账、后者是货,中间隔着一堆没人记的买入。**先盘一次,才谈得上差多少。**
- **⏳ 我没有自己去开那两个被 Disabled 的任务** —— 它们是被人主动关掉的,重开是个决定不是清理。**要开我随时能开;但只开提醒不解决问题,因为 7/25 之后提醒响了两周也没人盘。**

## 🔴 9/4 夜 退格符坑第六次,而这次它伪装成「这个改动没用」

- 我用 bash heredoc 往 `find_product_images.py` 里写正则,**`\b` 到 Python 手上时已经是字面的 U+0008 退格符**:文件里真实长这样 `re.sub(r"(?i)\x08PC\x08", ...)`。**这种正则匹配不了任何东西。**
- **最恶心的地方:失败和「没效果」长得一模一样。** 改完重跑,**5 个匹配、51 个未匹配,和上一轮一字不差** —— 读起来完全像「这个思路不行」,而不是「这段代码根本没跑」。**要不是我觉得「一字不差」太巧了去查文件,就会得出一个完全错误的结论然后放弃这条路。**
- 退格符**在终端里不可见** —— Read 出来正常、grep 出来也像正常代码,只有 `repr()` 才看得见。**而且 Edit 工具也改不动它**(我的匹配串里没有那些不可见字符),只能用 Write 写一个修复脚本,并让它**在文件里还剩任何一个 U+0008 时直接 abort**。
- **手册那条铁律本来就写着「这条管道改 .py 只用 Write/Edit,heredoc 里的反斜杠一律 `chr(92)` 构造」** —— 我为了省事用了 heredoc。现在那几行是 `B = chr(92) + "b"` 拼出来的,任何 shell 都碰不到。

## ✅ 9/4 晚 缩略图开始补(Gary:「缩略图开始找」→「go」,顺序 EN → JP → CN)

- 新 `add_product_image.py`:`<uuid8>=tcg:<产品id>` 或直接 URL,**写之前先取字节验是不是真图**(好几个图床会 200 回 HTML)· **已有图的绝不覆盖** · 尺寸 400→1000→200 逐个试(403 的尺寸不是小图,是没有图)· 同时写一份 `data/product_image_sources.json` provenance 台账,**记下这张图哪来的、怎么验的**,以后发现配错了能追回来而不是吵。
- **✅ 首批 6 个已发布,覆盖在架 453 件**:`OP-17 Blister 161`(704755,**下载下来亲眼看过是英文版 OP-17 挂卡**)· `4th Anniversary Treasure Campaign 108`(714348)· `[EN] OP-16 散包 86`(689341)· `Double Pack Set Vol.12 59`(704757)· `[EN] OP-08 盒 20`(542504)· `[EN] OP-17 盒 19`(704752)。
- **在架覆盖率 118 → 124 / 208(57% → 60%)**,feed 357 条,公网已生效。
- **✅ 第二批 7 个**:`Black Bolt 散包 [JP] 45`(636736,**正是早上被语言闸门拦下的那个** —— 那个名字下唯一的钉是英文版,而我们那行是日文的)· `Lorcana Wilds Unknown Sleeved 22`(682040)· 5 个 blister/collection 小件。**在架覆盖 57% → 62%(130/209)**,feed 364 条。
- **✅ 新 `find_product_images.py`:批量跑 TCG 搜索 + 一个故意很凶的匹配器**(形态必须一致 · 语言必须一致 · 我们套名里每个特征词都要出现 · **只剩一个候选才算数**),再验字节才写。
- **🔴 但 EN 那 56 个只中 5 个,而且瓶颈不在匹配器 —— 在我们自己的名字。**
  - **查询要先展开我们的缩写**:TCG 从没听说过 `PC ETB`(它叫 Pokemon Center Elite Trainer Box)、标题里也**从不带 `OP-02` 这种套号**、而 `Other` 是我们拼名字粘上的尾巴。**只把这些词从比对里剔掉不够 —— 拿原样缩写去搜,正确的产品压根不进结果集,匹配器连拒绝的机会都没有。**
  - 修完之后结果集确实对了(`Ascended Heroes PC ETB` 从 2 条变 25 条),**但过闸的仍是 5 个** —— 剩下的卡在两类真问题上。
- **📌 剩下 79 个,分两类,处方完全不同**:
  - **15 个 / 28 件只卡在「是哪一款」** —— `151 Tin` 13 个候选、`Ascended Heroes Tin` 3 个、4 个海贼王起始牌组各 2 个(牌组 vs 牌组展示盒)。**我们的名字里没写是哪款,猜就是 50% 配错。人扫一眼几秒钟的事。**
  - **其余是名字对不上** —— `[EN] OP-10 Royal Lineage` 而 TCG 那个套叫 **Royal Blood**;`Lost Orgin ETB` 是我们自己打错字。**这类要改的是我们的名字,不是匹配器。**
- **✅ 歧义那批已发 Frank(Gary:「可以发给frank」)**,在架覆盖 **134/209(64%)**。
  - **对照页 `https://lv-slabs.luckyvault.us/kaitori/sku_image_check.html`** —— 每个产品一行、候选图并排、按 A/B/C 标号,回一句 `1B 2A 3 none` 就行。
  - **不用 Artifact 用我们自己的静态托管**:Frank 在另一个 Lark 租户,Artifact 默认私有,**给他一个打不开的链接等于没发**。页面上**不放数量不放成本**,只有产品名和 TCG 公开产品图。
  - **🔴 先自己打开看了才发的**(8/13 那条铁律):390px 手机 + 1280 桌面,**横向溢出 0px、坏图 0 张**、控制台干净。而且**正是看截图才发现有 4 行根本不该问人** —— 海贼王起始牌组的两个候选是「牌组」和「牌组的 6 个装展示盒」(TCG 图上直接印着 x6),而我们的名字里没有 display 字样。**已自动定掉,Frank 的清单 15 → 11。**
  - **`autoresolve_variants.py` 的规则和它的边界**:只有当**我们的名字不含 display/case/bundle 且候选里恰好只有一个也不含**时才自动定;我们自己就是 display 的、或者剩两个都是素的,一律留给人。**问人的每一条都必须是只有人能答的 —— 拿数据能答的问题去凑清单,是让清单彻底没人回的最快方法。**
  - ⚠️ 我一度以为当时是凌晨 3 点、打算静音发,**实际是 PT 上午 11:23,正常发的**。判「现在几点」别从日志里那个 `02:18` 反推,那是别的东西。
- **⏳ 还剩 75 个 / 约 740 件。** 头部仍是没有 TCG 线的:`Gem Pack Vol 6 [CN] 135` · `Rarity Collection [JP] 116` · `Nivel Arena [JP] 71` —— **CN 在 TCG 上完全不存在,JP 冷门套也多半没有**,要走官方站/eBay,每一张都要人过目。

## ✅ 9/4 成本切换点已冻结:今天的成本定死成第 0 层,FIFO 从明天往前长(Gary:「我们现在的 cost 定死就行 从现在开始补 fifo」)

- **这是标准的 cutover,而且在这里是对的**:往回重建层是不可能的 —— **89 个在架产品完全没有进货历史、13 个在架比历来买进还多**,而切换点之前的消耗是一个没有顺序的盘点残差。所以不再试:**今天在架的每一件都变成一个开账层,单价用账面已经在用的成本;明天起 FIFO 在它上面累积。**
- **✅ 已冻结(`cost_layer_freeze.py --write`,一次性)**:`2026-09-04` · **208 个产品 / 16,139 件 / $182,283**(和账面一分不差)· 记下**冻结时刻的 929 个 acquisition id**。
  - **这一条是成败所在:切换点之前的进货已经在第 0 层里了,再记一次层就是重复计。** 所以只有**冻结之后才出现**的行才能成层;而**冻结之后出现、日期却在切换点之前**的(补录的老单)**不成层,单独列出来**,绝不静默吸收。
  - **文件不可改**:重跑直接拒绝(实测),并存了 sha256,**看门狗每天验一次自己的校验和** —— 开账层一动,自它冻结以来的每一个 COGS 都被追溯改写,而别的检查一个都不会察觉,因为层会和自己自洽。
- **覆盖率 64% → 100%**:`FIFO 算得出来 208/208 个产品,16,139/16,139 件`。**第一天 FIFO = 账面 = $182,283,差 $0 —— 这是构造上必然的,报告里明写了**,不是「验证通过」。
- **⚠️ 冻结把今天的错也一起定死了,这话要说在前面**:**541 件完全没有成本的冻在 $0**(不是估值,是一个已知的洞,所以层上带 `cost_known=false`、永远显示 $0,不许被平均掉,烧出去时按 $0 计 COGS 并且报告会说)。另外**按进货史看,`One Piece Asia exclusive Mini-tin TS-03` 49 罐账面比 8 月实付低了约 $7,417** —— 冻结之前它是隐形的,冻结之后它是一个可以指着看的层,**要不要用一笔调整层改过来,你说了算**。
- **📌 判断 on track 就盯一个数:「还坐在第 0 层上的件数」。** 今天 **16,139 / 16,139 = 100%**。**一个产品只要还在第 0 层,FIFO 和加权平均对它就必然给同一个答案** —— 这个比例往下掉,才是真的 FIFO 在长出来。
- **看门狗加了三条新判据**(共 8 类):**开账文件被改**(校验和对不上)· **两天用了不同的开账**· **补录的切换点前进货**(可能是真新单但日期打错,不许自动吞掉)。
- **🔴 测试 13 项全过,而且做了两轮变异**:关掉「层被改」→ 3 条红;关掉「开账校验和 + 补录检测」→ 2 条红。测试全程不碰真数据、不发 Telegram。
  - ⚠️ 测试自己踩了两个坑,都值得记:① 把开账文件写进了台账目录 → `*.json` 全被当成日期,`fromisoformat('opening')` 直接把看门狗**炸掉**。已把 `entries()` 收紧成只认 `YYYY-MM-DD.json` —— **一条会被一个乱入文件搞死的看门狗,是关掉了却不说话的看门狗**;② 快照先写、开账后写,于是每个快照都指着旧哈希,**「开账被换掉」在每个用例里都响,把真正要测的那条盖住了**。

## ✅ 9/4 成本层台账 + 看门狗上线(Gary:「go 从现在开始 并且加上 watchdog 去监控」「每天可以跑一次 确保我们 on track」)

- **口径一个字没改 —— 记账仍然是加权平均。** 新 `cost_layers.py` 只**在影子里**算 FIFO 并写一份带日期的台账。理由:**唯一补不回来的是历史**。`inventory` 没有 audit log,数量一被覆盖,它走过的路就永远没了 —— **今天不快照的每一天,都是 FIFO 以后永远算不出来的一天。**
- **而消耗还不能撑起 FIFO,这是量出来的**:`box_breaks` 全库 **2 行** · `platform_sales` **157 行**(7/27 之后死的)· `movements` 2,530 行里 **2,525 是换房不是消耗** —— 记录消耗的主力是**盘点残差:负差写掉 50,970 件,同时丢弃 15,567 件正差**。**残差没有顺序,所以「烧的是哪一层」根本没有答案;加权平均正是「我不知道是哪层」的诚实表达,而 FIFO 会给出一个精确但是编的数字。**
- **算法就是 Gary 说的那句**:累计消耗 = 历来买进 − 现在在架,拿它从最老的层开始烧,剩下的就是当前层。**做在公司级不做房间级** —— 换房不消耗,而 2,525/2,530 的 movements 都是换房,按房分层要在每一笔上劈层,而那批数据本来就脏。
- **首日基线(已发 Telegram,证明这条管道是通的不是只 import 得动)**:
  ```
  FIFO 算得出来   201 / 313 个产品   14,146 / 16,139 件
  FIFO $141,299   vs   账面 $135,256   差 $6,042(+4.5%)
  算不出来的 112 个:89 个完全没有进货历史(1,018 件)· 13 个在架比历来买进还多(917 件)· 10 个层上没有成本(58 件)
  ```
  **最大一条分歧是 `One Piece Asia exclusive Mini-tin TS-03` +$7,417 / 49 罐** —— 8 月花 $8,900 买的那批(≈$189/罐),账面把它记成了大约四分之一。**单独一条就占了整个 $6,042 差额还有余,说明差额不是「方法不同」而是「有几行账面是错的」。**
- **✅ `cost_layer_watch.py` + 任务 `LV Cost Layers`(每天 06:10)已上线并实跑通过**(exit 0)。**顺序写死:先快照再看门** —— 反过来会拿昨天比前天,**每天都报「一切正常」,一条结构上永远不可能响的看门狗**。
- **它盯的五件事,每一件都会悄悄毁掉层历史**:① **层被事后改**(改一条老进货的成本或数量,此后所有 FIFO 全部静默失真 —— `fix_unit_costs.py` 差一个 `--write` 就干这件事,而 8/12 那条 OP-16 就是几个月后被手改的)② **层消失** ③ **台账断天**(断的那几天顺序永久没了)④ **在架比买进还多的缺口在扩大** ⑤ **新层没有成本**。
- **🔴 十个分支逐个实测,而且做了变异检查**:`test_cost_layer_watch.py` 拿伪造的两天喂真脚本,**10/10 过**,其中两条是「必须闭嘴」的对照(什么都没变 / 缺口在缩小)。**把「层被改」那条判据关掉 → 当场 3 条变红**,证明测试真的会响。测试全程不碰真数据、不发 Telegram。
- ⚠️ 层的数量取 `quantity_purchased` 不取 `quantity_received` —— 后者全系统基本没人维护(5 月的单还挂 0),用它会把大部分层直接抹掉。**两个都写进台账**,哪天 received 可信了就能换过来重算。
...
### ✅ 9/4 晚追加(Gary:「pack是单包 in bag 是上面写个x30」)—— 三处已改并生效
- **Storm Emeralda (In Bag) 71 件补上 ×30**,底图用 **Storm 的单包图**(709111)不是盒图 —— **一袋是把盒扔了的 30 个散包,所以诚实的画面是包,数字压在上面**;用盒图会让它在点货表上和旁边那 179 件封盒长得一模一样,而这两行差 30 倍。新 `x30_badge.py`,**数字只从 `packs_per_box` 取,取不到就不画**(Mega Dream 是 10 包的 High Class Pack,7/20 就栽过一次)。
  - ⚠️ 第一版把 504×1000 的底图直接 resize 成 400×400,**把包压扁了一半**,而 `object-cover` 会让人只看到一个胖包看不出是 bug。已改成**补边成正方形再缩**。
  - **`(Unsealed)` 故意不加** —— 拆封盒仍然是一个盒、货架上看起来也是盒,盒图对它是诚实的。周巡的判据同步收窄到 `in_bag` / `open`。
- **🔴 `Abyss Eye Booster Box`(32 件)一直在显示单包的照片,而正确的盒图就躺在隔壁没人用。** 病根是 uuid8 撞车 + `m[uuid8]=图` **后写的赢**:两条 Shopify listing 都挂 `748116b5`,散包那条最后加载 → 盒行拿到 `695111`(实测确认是单包图),而盒图 `695112` **早就钉在 `sku_urls.json` 里、也就在盒 listing 上**。已改成**撞车时选标题形态和产品对得上的那条**(`build_product_images.py`),Abyss Eye + Black Bolt 两行当场修好,**变化 5 条、丢失 0 条**。
- **🔴 而 uuid8 撞车本身没修 —— 那要在 Shopify 侧改 SKU,不是图这边。** 散包 listing 背着盒的 uuid8,**销售和重定价也是按这个 uuid8 回指产品的**(8/07 记的「同一个成本会同时驱动两条价」)。

### ⚠️ 周巡自己犯了两个错,都是靠真数据抓回来的
- **① 按 URL 字符串比重复,漏掉了最大的一组**:`Abyss Eye Booster Box` 用 Shopify 的 `695111.png?v=…`,`Abyss Eye Single Pack` 用 TCG 的 `695111_in_400x400.jpg` —— **两个 URL、同一张照片**(Shopify 那些图当初就是拿 TCG 图传上去的,数字 id 留在文件名里)。**屏幕上 32 个盒和 1,539 个包是同一张图。** 已改成按**图片身份**比,重复 6 → 15 条。
- **② `SHOPIFY-FORM` 五条全是冤枉的**:我拿「所有挂这个 uuid8 的 listing」去比标题,而 Uma Musume / Dual Evolution / Time of Battle 那几条散包 listing **根本没有图**,builder 会跳过,所以盒行拿的一直是盒图。已改成**只审真正提供了那张图的 listing**,5 → 0。**一份会喊狼来了的周报,第三周就没人点开了 —— 这句是我自己写的,当天就差点自己犯。**
...
- **🔴 9/6 归属定了,这一节的框架要按新的读**(见开头那条):**Mystery Game 是 ebay2 系列盲盒的收入,不是门店的。** 这一节做对的是「从门店剔出去」,**漏掉的是没人接住它** —— 三张表合计 **$133,894** 剔出去之后落在了空气里。**「剔除」要改成「改记到 ebay2」;在那之前 ebay2 系统性低报,门店是干净的。** 另外 `location_id` 不能当归属用:打标行按 location 记着 Master $22,052 / Front $17,177,那是**货从哪拿**不是**卖在哪**。
